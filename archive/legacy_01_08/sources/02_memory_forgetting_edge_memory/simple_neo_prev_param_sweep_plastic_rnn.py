"""
Constructive-ranking baseline with optional plastic Hebbian RNN core.

This script is a command-line-controllable extension of
simple_neo_prev_param_sweep.py.  It keeps the original score-coordinate
constructive learner as the default baseline, and adds an optional episode-local
plastic RNN/Hebbian fast-weight module for the current no-feedback ranking task.

Design:
  - Outer loop: supervised meta-loss after a full learning episode.
  - Inner loop: passive observation-only pair learning; no response/reward.
  - Test phase: no feedback; choices read from the learned scalar score axis.
  - Plastic state reset: reset at episode/virtual-subject start; keep through
    all learning blocks and into testing; discard at episode end.

Typical comparison:
  python simple_neo_prev_param_sweep_plastic_rnn_2_1.py --no-plastic-rnn ...
  python simple_neo_prev_param_sweep_plastic_rnn_2_1.py --use-plastic-rnn ...
"""
...
# Rank-space pairs from the behavioral task. Ranks 0..7 are low-to-high.
# (0, 5) means A-F, and the oriented relation A->F has signed distance +5/7.
...
class TrainConfig:
    ...
    save_every: int = 50
    pe: int = 10
    output_dir: str = "outputs_constructive_plastic"
    num_threads: int = 1
    ...
    # Baseline constructive score network
    hidden_size: int = 8
    item_dim: int = 15
    subject_dim: int = 8
    ...
    disable_hebbian_plasticity: bool = False
...
class PlasticHebbianCore(nn.Module):
    """Episode-local plastic RNN core.

    The trainable slow weights are normal module parameters. The hidden state and
    plastic matrix are not parameters; they are reset per episode and updated for
    each passive observation. They are kept across blocks and into testing.
    """
    ...
    def initial_state(self, bsz: int, device: torch.device | str) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def step(
        ...
        if cfg.detach_plastic_state:
            h_for_fast = h.detach()
            plastic_for_fast = plastic.detach()
        else:
            h_for_fast = h
            plastic_for_fast = plastic

        fast = torch.bmm(plastic_for_fast, h_for_fast.unsqueeze(2)).squeeze(2)
        pre = (
            cfg.rnn_input_gain * self.input_proj(x)
            + cfg.rnn_rec_gain * self.rec_proj(h)
            + cfg.fast_weight_gain * fast
        )
        h_new = torch.tanh(self.layer_norm(pre))
        if train_mode and cfg.rnn_dropout > 0:
            h_new = self.dropout(h_new)

        if cfg.disable_hebbian_plasticity:
            eta = torch.zeros(h_new.shape[0], device=h_new.device)
            plastic_new = plastic
        else:
            eta = cfg.hebb_eta * torch.sigmoid(self.eta_head(torch.cat([x, h_new], dim=1))).squeeze(1)
            hebb = torch.bmm(h_new.unsqueeze(2), h.unsqueeze(1))
            plastic_new = cfg.plastic_decay * plastic + eta[:, None, None] * hebb
...
class ConstructiveRankingNet(nn.Module):
    """Score-memory updater with optional plastic RNN features in update_gate."""
    ...
    def __init__(self, config: TrainConfig):
        ...
        self.update_gate = nn.Sequential(
            nn.Linear(self.base_feature_dim + rnn_out_dim, h),
            nn.Tanh(),
            nn.Linear(h, h),
            nn.Tanh(),
            nn.Linear(h, 1),
    ...
    def initial_recurrent_state(self, bsz: int, device: torch.device | str) -> tuple[torch.Tensor | None, torch.Tensor | None]:
    ...
    def build_base_features(
        ...
        return base_features, pred_diff, error
    ...
    def update_scores(
        ...
        bsz = scores.shape[0]
        batch_idx = torch.arange(bsz, device=scores.device)
        base_features, pred_diff, error = self.build_base_features(
            scores, item_vecs, subject_z, item_i, item_j, observed_diff_j_minus_i
        )

        eta = torch.zeros(bsz, device=scores.device)
        gate_features = base_features
        if self.rnn_core is not None:
            assert rnn_h is not None and plastic is not None
            rnn_h, plastic, eta = self.rnn_core.step(base_features, rnn_h, plastic, train_mode=train_mode)
            gate_features = torch.cat([base_features, rnn_h], dim=1)

        gate = torch.sigmoid(self.update_gate(gate_features)).squeeze(1)
...
def run_learning_phase(
    ...
    rnn_h, plastic = net.initial_recurrent_state(bsz, scores.device)
    ...
    for _block in range(config.n_learning_blocks):
        ...
        for k in range(n_edges):
            ...
            scores, pred_diff, error, gate, rnn_h, plastic, eta = net.update_scores(
                scores, item_vecs, subject_z, item_i, item_j, obs_diff, rnn_h, plastic, train_mode=train_mode
            )
            recon_losses.append(error.pow(2).mean())
            gates.append(gate.mean())
            etas.append(eta.mean())
            if plastic is not None:
                plastic_abs.append(plastic.abs().mean())

    recon_loss = torch.stack(recon_losses).mean() if recon_losses else torch.tensor(0.0, device=DEVICE)
    mean_gate = torch.stack(gates).mean() if gates else torch.tensor(0.0, device=DEVICE)
    mean_eta = torch.stack(etas).mean() if etas else torch.tensor(0.0, device=DEVICE)
    mean_abs_plastic = torch.stack(plastic_abs).mean() if plastic_abs else torch.tensor(0.0, device=DEVICE)
    return scores, recon_loss, mean_gate, mean_eta, mean_abs_plastic, (plastic.pow(2).mean() if plastic is not None else torch.tensor(0.0, device=DEVICE))
...
def run_training_episode(config: TrainConfig, net: ConstructiveRankingNet) -> EpisodeStats:
    ...
    scores, recon_loss, mean_gate, mean_eta, mean_abs_plastic, plastic_l2 = run_learning_phase(
    ...
    loss = (
        ...
        + config.lambda_plastic_l2 * plastic_l2
        + config.lambda_eta_l2 * mean_eta.pow(2)
    )
    ...
    return EpisodeStats(
        ...
        mean_hebb_eta=float(mean_eta.detach()),
        mean_abs_plastic=float(mean_abs_plastic.detach()),
...
def train(config: TrainConfig) -> ConstructiveRankingNet:
    ...
    log(f"[setup] use_plastic_rnn={config.use_plastic_rnn}; disable_hebbian_plasticity={config.disable_hebbian_plasticity}")
    ...
    log("[task] Inner-loop reward: none. Learning trials provide relation constraints only.")

    log_path = output_dir / "train_log.csv"
    fieldnames = [
        "episode", "loss", "choice_loss", "recon_loss", "entropy_loss", "test_acc", "learned_acc",
        "nonlearned_acc", "mean_gate", "mean_hebb_eta", "mean_abs_plastic", "mean_abs_score",
    ]
    ...
    for episode in episode_iter:
        ...
        if episode % config.pe == 0 or episode == config.nbiter - 1:
            ...
            log(
                ...
                f"gate={stats.mean_gate:.3f} eta={stats.mean_hebb_eta:.4f} |plastic|={stats.mean_abs_plastic:.4f} |score|={stats.mean_abs_score:.3f}"
            )
            start = time.time()
        ...
        with open(log_path, "a", newline="") as f:
            ...
            writer.writerow([
                ...
                stats.test_acc, stats.learned_acc, stats.nonlearned_acc, stats.mean_gate,
                stats.mean_hebb_eta, stats.mean_abs_plastic, stats.mean_abs_score,
            ])

        if config.save_every > 0 and episode > 0 and episode % config.save_every == 0:
            torch.save({"model_state": net.state_dict(), "config": asdict(config)}, output_dir / "net_constructive_plastic.pt")
            log(f"[save] checkpoint: {output_dir / 'net_constructive_plastic.pt'}")

    torch.save({"model_state": net.state_dict(), "config": asdict(config)}, output_dir / "net_constructive_plastic.pt")
    log(f"[save] final checkpoint: {output_dir / 'net_constructive_plastic.pt'}")
...
def evaluate_paper_task(config: TrainConfig, net: ConstructiveRankingNet) -> dict:
    ...
    scores, recon_loss, mean_gate, mean_eta, mean_abs_plastic, _plastic_l2 = run_learning_phase(
        config, net, item_vecs, subject_z, true_scores, learning_edges, train_mode=False
    )
    ...
    for pidx, (ra, rb) in enumerate(pair_list):
        ...
        labels = torch.ones(n_subj, dtype=torch.long, device=DEVICE)
        logits = net.choice_logits(scores, item_i, item_j)
    ...
    summary = {
        ...
        "use_plastic_rnn": config.use_plastic_rnn,
        "disable_hebbian_plasticity": config.disable_hebbian_plasticity,
        ...
        "mean_hebb_eta": float(mean_eta.cpu()),
        "mean_abs_plastic_weight": float(mean_abs_plastic.cpu()),
        "mean_abs_final_score": float(scores.abs().mean().cpu()),
        "mean_choice_logit_diff_rank_ordered": float(np.mean(beta_logits_all)),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Constructive ranking baseline with optional plastic Hebbian RNN.")

    parser.add_argument("--nbiter", type=int, default=4)
    parser.add_argument("--batch-size", type=int, default=2)
    parser.add_argument("--hidden-size", type=int, default=8, help="MLP hidden size for score_init/update_gate.")
    parser.add_argument("--item-dim", type=int, default=15)
    parser.add_argument("--subject-dim", type=int, default=8)
    parser.add_argument("--seed", type=int, default=1)
    parser.add_argument("--output-dir", default="outputs_constructive_plastic")
    parser.add_argument("--save-every", type=int, default=50)
    parser.add_argument("--print-every", type=int, default=10)
    parser.add_argument("--num-threads", type=int, default=1, help="CPU threads for PyTorch; 1 is often fastest for many small ops.")
    ...
    parser.add_argument("--paper-graph-train-prob", type=float, default=0.0)
    parser.add_argument("--eval-beta-override", type=float, default=0.0)
    parser.add_argument("--lr", type=float, default=3e-4)
    parser.add_argument("--grad-clip", type=float, default=2.0)

    rnn_group = parser.add_mutually_exclusive_group()
    rnn_group.add_argument("--use-plastic-rnn", dest="use_plastic_rnn", action="store_true", help="Enable plastic Hebbian RNN core.")
    rnn_group.add_argument("--no-plastic-rnn", dest="use_plastic_rnn", action="store_false", help="Disable plastic Hebbian RNN core.")
    parser.set_defaults(use_plastic_rnn=False)
    parser.add_argument("--rnn-hidden-size", type=int, default=64)
    parser.add_argument("--rnn-input-gain", type=float, default=1.0)
    parser.add_argument("--rnn-rec-gain", type=float, default=1.0)
    parser.add_argument("--fast-weight-gain", type=float, default=0.35)
    parser.add_argument("--hebb-eta", type=float, default=0.06)
    parser.add_argument("--plastic-decay", type=float, default=0.96)
    parser.add_argument("--plastic-clip", type=float, default=3.0)
    parser.add_argument("--disable-hebbian-plasticity", action="store_true", help="Keep vanilla RNN hidden state but freeze fast weights.")
    parser.add_argument("--detach-plastic-state", action="store_true", help="Detach recurrent/plastic state before fast-weight read; faster/less memory, less meta-gradient.")
    ...
    parser.add_argument("--lambda-score-l2", type=float, default=1e-4)
    parser.add_argument("--lambda-plastic-l2", type=float, default=1e-5)
    parser.add_argument("--lambda-eta-l2", type=float, default=1e-4)
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    return TrainConfig(
        ...
        paper_graph_train_prob=args.paper_graph_train_prob,
        eval_beta_override=args.eval_beta_override,
        lr=args.lr,
        gc=args.grad_clip,
        use_plastic_rnn=args.use_plastic_rnn,
        rnn_hidden_size=args.rnn_hidden_size,
        rnn_input_gain=args.rnn_input_gain,
        rnn_rec_gain=args.rnn_rec_gain,
        fast_weight_gain=args.fast_weight_gain,
        hebb_eta=args.hebb_eta,
        plastic_decay=args.plastic_decay,
        plastic_clip=args.plastic_clip,
        disable_hebbian_plasticity=args.disable_hebbian_plasticity,
        detach_plastic_state=args.detach_plastic_state,
        rnn_dropout=args.rnn_dropout,
        lambda_recon=args.lambda_recon,
        lambda_entropy=args.lambda_entropy,
        lambda_score_l2=args.lambda_score_l2,
        lambda_plastic_l2=args.lambda_plastic_l2,
        lambda_eta_l2=args.lambda_eta_l2,
    )
...
def main() -> None:
    ...
    output_dir = Path(config.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    with open(output_dir / "config_constructive_plastic.json", "w") as f:
