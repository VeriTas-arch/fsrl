...
from .batches import selected_generic
...
def _generic(
    ...
    for name, record in sorted(inputs.items()):
        ...
        if keep_p:
            selected_cpu, selected = selected_generic(cpu)
            states, _, modulations, _ = support_trajectory(model, selected_cpu)
            internal[name] = {
                "terminal_P": terminal.cpu().numpy(),
                "prefix_P": torch.stack(states).cpu().numpy(),
                "natural_modulation": torch.stack(modulations).cpu().numpy(),
                "selected_positions": selected.astype(np.int64),
                "selected_episode_indices": selected_cpu.arrays["episode_indices"],
            }
        del terminal
...
def _liu(model, record: dict, observation: str, recipe: dict, keep_p: bool):
    ...
    internal = {"terminal_P": terminal.cpu().numpy()} if keep_p else {}
