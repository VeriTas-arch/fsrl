...
def planned_power(n: int, standard_deviation: float, sesoi: float) -> dict:
    ...
    def integrand(variance_draw: float) -> float:
        ...
        conditional = max(
            0.0, 2.0 * float(stats.norm.cdf(standardized_bound)) - 1.0
        )
...
def _assignment_geometry(specification: dict) -> dict:
    ...
    all_pairs = [
        canonical_pair(first, second, items)
...
def validate_specification(specification: dict) -> dict:
    ...
    for name, assignment in assignments.items():
        assignment_gates[f"{name}_rank_level_permutation"] = sorted(
            assignment["rank_levels"].values()
        ) == expected_levels
        ...
        assignment_gates[f"{name}_registered_gaps_match_levels"] = (
            assignment["registered_support_gaps"]
            == assignment["derived_support_gaps"]
        )
    ...
    registered_distance_change = {
        pair: int(value)
        for pair, value in frozen[
            "same_direction_distance_change_B_minus_A"
        ].items()
    }
    ...
    assignment_gates["magnitude_change_vector"] = (
        registered_magnitude_change
        == geometry["learned_magnitude_change_B_minus_A"]
    )
    ...
    task_gates = {
        ...
        "disjoint_image_sets_registered": "disjoint image sets"
        in design["design"],
        ...
        and support["trials"]
        == support["relations_per_block"] * support["blocks"],
        ...
        "all_pairs_are_queried": query["pairs_per_block"]
        == len(geometry["all_pairs"]),
    }
    ...
    analysis_gates = {
        ...
        and "step_2_nonlearned_confidence"
        in specification["hierarchical_outcome_tree"]
        and "step_3_learned_relations"
        in specification["hierarchical_outcome_tree"],
    }
