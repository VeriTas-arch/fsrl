...
def validate_sources(
    ...
    *,
    ...
    passed = all(value is True or value == "deferred_without_opening" for value in checks.values())
    expected_opened = ["derivation_trials"] if phase == "derive" else [
        "derivation_trials",
        "confirmation_trials",
    ]
...
def pair_metadata(protocol: RankingProtocol) -> dict:
    ...
    positions[np.asarray(protocol.true_order_high_to_low)] = np.arange(
        protocol.n_items
    )
    labels = tuple(
    ...
    true_higher = np.asarray(
        [first if positions[first] < positions[second] else second for first, second in pairs]
    )
    expected_labels = (
        "A-B",
        "A-C",
...
def model_arrays(protocol: RankingProtocol, specification: dict) -> dict:
    ...
    masks = (
        (mask_ids[:, None] >> np.arange(len(expected_relations))) & 1
    ).astype(np.float64)
...
def load_trial_cohort(
    ...
    *,
    ...
    for source_id, rows in sorted(grouped.items()):
        ...
        if len(rows) != 280 or len(identifiers) != 280 or not np.all(np.isfinite(matrix)):
...
def torch_order_distribution(
    ...
    *,
    ...
    log_mask = mask_counts * torch.log(torch.as_tensor(rho, dtype=torch.float64, device=device))
...
class DerivationObjective:
    ...
    def __call__(self, values: np.ndarray) -> tuple[float, np.ndarray]:
        ...
        base = torch.sum(
            self.correct_counts * log_bad
            + (10.0 - self.correct_counts) * log_good,
            dim=1,
        )
        delta = (2.0 * self.correct_counts - 10.0) * (log_good - log_bad)
        response = (
            base[:, None]
            + delta @ self.order_correct.T
            + self.log_binomial[:, None]
        )
...
def fit_parameters(
    ...
    *,
    ...
    if (
        len(starts) != 27
        or optimizer_contract["starts"]["cartesian_total"] != 27
    ):
    ...
    if converged:
        ...
        near_field_error = max(
            float(
                np.max(
                    np.abs(np.asarray(record["field"], dtype=np.float64) - selected_field)
                )
            )
            for record in near
        )
...
def row_correlations(first: np.ndarray, second: np.ndarray, minimum: float) -> tuple[np.ndarray, np.ndarray]:
...
def behavioral_components(
    ...
    *,
    ...
    true_first = np.asarray(
        [higher == pair[0] for higher, pair in zip(metadata["true_higher"], metadata["pairs"], strict=True)]
    )
    ...
    metrics = {
        ...
        "mean_self_consistency_coefficient": float(
            np.mean(self_consistency[eligible])
        ),
...
def human_qualification_bootstrap(components: dict, counts: np.ndarray) -> dict:
    ...
    self_consistency = (
        counts @ (components["self_consistency"] * eligible) / eligible_n
    )
...
def predictive_qualification(
    ...
    human_components = behavioral_components(
        np.sum(human_trials, axis=1), arrays
    )
    ...
    integrity = {
        ...
        "human_eligible_subjects": human_components["metrics"][
            "eligible_subjects"
        ],
        "human_analysis_subjects": human_components["metrics"][
            "analysis_subjects"
        ],
...
def derive(
    ...
    parameter_lock = {
        ...
        "implementation_lock_sha256": file_sha256(
            DEFAULT_IMPLEMENTATION_LOCK_PATH
        ),
        "parameter_artifact": {
...
def main(args=None) -> int:
    ...
    if parsed.phase == "derive":
        ...
        parameter_lock["parameter_artifact"]["sha256"] = file_sha256(
            parsed.parameters
        )
