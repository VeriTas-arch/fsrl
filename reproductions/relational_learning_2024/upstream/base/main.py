...
params["wp"] = 0.0  # penalty for taking action 1 (not used here)
params["bent"] = 0.1  #  entropy incentive (actually sum-of-squares)
params["blossv"] = 0.1  # value prediction loss coefficient
params["gr"] = 0.9  # Gamma for temporal reward discounting
...
params["triallen"] = (
    4  # 4  #  5 # 5 + 1 #  4 + 1  # Each trial has: stimulus presentation, 'go' cue, then 3 empty trials.
)
NUMRESPONSESTEP = 1
params["nbtraintrials"] = (
    20  # 22  # 20 #  5  #  The first  nbtraintrials are the "train" trials. This  is included in nbtrials.
)
params["nbtesttrials"] = (
    10  # 2 #  12 # 10  #  The last nbtesttrials are the "test" trials. This  is included in nbtrials.
)
params["nbtrials"] = (
    params["nbtraintrials"] + NBMASSEDTRIALS + params["nbtesttrials"]
)  #  20   # Number of trials per episode
...
class RetroModulRNN(nn.Module):
    def __init__(self, params):
        ...
        for paramname in ["outputsize", "inputsize", "hs", "bs"]:
            if paramname not in params:
                raise KeyError(
                    "Must provide missing key in argument 'params': " + paramname
                )
        ...
        self.w = torch.nn.Parameter(
            (
                (1.0 / np.sqrt(params["hs"]))
                * (2.0 * torch.rand(params["hs"], params["hs"]) - 1.0)
            ).to(device),
            requires_grad=True,
        )
        ...
        if SCALARALPHA:
            self.alpha = (
                0.01 * (2.0 * torch.rand(1, 1) - 1.0).to(device)
            )  # # A single scalar, so all connections share the same single plasticity coefficient (still trained)
        elif VECTALPHA:
            self.alpha = (
                0.01 * (2.0 * torch.rand(params["hs"], 1) - 1.0).to(device)
            )  # A column vector, so each neuron has a single plasticity coefficient applied to all its input connections
        else:
            self.alpha = 0.01 * (2.0 * torch.rand(params["hs"], params["hs"]) - 1.0).to(
                device
            )
        ...
        self.h2o = torch.nn.Linear(params["hs"], self.params["outputsize"]).to(
            device
        )  # Actual output
    ...
    def initialZeroET(self, mybs):
        ...
        return torch.zeros(
            mybs, self.params["hs"], self.params["hs"], requires_grad=False
        ).to(device)
...
suffix = (
    ...
    + "".join(
        [
            ...
            for kk, vv in sorted(params.items())
        ]
    )
    + "_rng"
    + str(params["rngseed"])
...
params["inputsize"] = (
    NBSTIMBITS + ADDINPUT + params["outputsize"]
...
LINKINGISSHAM = False
LINKED_PERFORMANCE_PATH = ROOT_DIR / "perfsperdist_link.npy"
...
assert not (
    (NBMASSEDTRIALS > 0) and (not EVAL)
)  # We should only use massed trials in eval, not training
if ONLYTWOLASTADJ:
    assert params["nbcues"] == 7
if HALFNOBARREDPAIRUNTILT18:
    assert (
        EVAL
        and (NBMASSEDTRIALS == 0)
        and not LINKEDLISTSEVAL
        and not ONLYTWOLASTADJ
        and not FIXEDCUES
    )
if LINKINGISSHAM:
...
for numepisode in range(params["nbiter"]):
    ...
    if LINKEDLISTSEVAL and numepisode == 2:
        ...
        params["nbtrials"] = params["nbtraintrials"] + params["nbtesttrials"]
        params["eplen"] = (
            params["nbtrials"] * params["triallen"]
    ...
    numactionschosen_alltrialsandsteps_thisep = np.zeros(
        (BS, params["nbtrials"], params["triallen"])
    ).astype(int)
    if EVAL:
        allpwsavs_thisep = []
        ds_thisep = []
                    ...
                    if nb == 0:
                        ...
                        while foundsame > 0:
                            ...
                            if cpt > 10000:
                                ...
                                raise ValueError(
                                    "Could not generate a full list of different cues"
                                )
                            foundsame = 0
    ...
    for numtrial in range(params["nbtrials"]):
        ...
        for nb in range(BS):
            ...
            if nbtrials[nb] < params["nbtraintrials"] or (
                ONLYTWOLASTADJ and nbtrials[nb] >= params["nbtrials"] - 2
            ):
                if ONLYTWOLASTADJ and nbtrials[nb] >= params["nbtrials"] - 2:
                    while (
                        abs(cuepair[0] - cuepair[1]) > 1 or 0 in cuepair or 6 in cuepair
                    ):
                        cuepair = list(np.random.choice(cuerange, 2, replace=False))
                else:
            ...
            if (
                NBMASSEDTRIALS > 0
                and nbtrials[nb] >= params["nbtraintrials"]
                and numtrial < params["nbtrials"] - params["nbtesttrials"]
            ):
                cuepair = MASSEDPAIR
            ...
            if (
                LINKEDLISTSEVAL
                and numepisode == 2
                and numtrial < params["nbtraintrials"]
            ):
                if LINKINGISSHAM:
                    cuepair = [
                        params["nbcues"] // 2 - 3,
                        params["nbcues"] // 2 - 2,
                    ]  # Sanity check for debugging: this should lead to chance perf in the test trial of 3rd episode
                else:
                    cuepair = (
            ...
            if nb > params["bs"] // 2 and HALFNOBARREDPAIRUNTILT18:
                if numtrial == 18:
                    cuepair = (
                        BARREDPAIR
                        if np.random.randint(2)
                        else [BARREDPAIR[1], BARREDPAIR[0]]
                    )
                elif numtrial < 18:
                    while True:
                        cuepair = list(np.random.choice(cuerange, 2, replace=False))
                        if abs(cuepair[0] - cuepair[1]) == 1 and (
                            (BARREDPAIR[0] not in cuepair)
                            or (BARREDPAIR[1] not in cuepair)
                        ):
                            break
            ...
            istest_thisep[nb, numtrial] = (
                1 if numtrial >= params["nbtraintrials"] + NBMASSEDTRIALS else 0
            )
            ...
            assert (
                mycues[NUMRESPONSESTEP] == params["nbcues"]
            )  # The 'response' step is signalled by the 'go' cue, whose number is params['nbcues'].
            cues[nb] = mycues
        ...
        for numstep in range(params["triallen"]):
            ...
            for nb in range(BS):
                ...
                if cues[nb][numstep] != -1 and cues[nb][numstep] != params["nbcues"]:
                    ...
                    if isolddata_thisep[nb, numtrial]:
                        ...
                        inputs[nb, : NBSTIMBITS - 1] = np.concatenate(
                            (
                                oldcuedata[oldpos][nb][cues[nb][numstep][0]][:],
                                oldcuedata[oldpos][nb][cues[nb][numstep][1]][:],
                            )
                        )
                    else:
                        inputs[nb, : NBSTIMBITS - 1] = np.concatenate(
                ...
                assert (
                    NUMRESPONSESTEP + 1 < params["triallen"]
            ...
            if PRINTTRACE:
                print(
                    ...
                    inputs[0, : params["cs"]],
                    "Cue 2 (0):",
                    inputs[0, params["cs"] : 2 * params["cs"]],
                    "Other inputs:",
                    inputs[0, 2 * params["cs"] :],
                    "\n - Outputs(0): ",
                    y.data.cpu().numpy()[0, :],
                    " - action chosen(0): ",
                    numactionschosen[0],
            ...
            for nb in range(BS):
                ...
                if numstep == NUMRESPONSESTEP:  # 2: # 4: #3: #  2:
                    ...
                    if (
                        cuepairs_thistrial[nb][0] < cuepairs_thistrial[nb][1]
                    ) and numactionschosen[nb] == 1:
            ...
            lossDA += torch.sum(
                torch.abs(DAout / (1e-8 + net.DAmult))
            )  # This is a hack to "remove" DAmult from the L1 penalty. Assumes DAmult never goes  to < 0.
            lossHL1 += torch.mean(torch.abs(hidden))

            loss += (
                params["bent"] * y.pow(2).sum() / BS
    ...
    for numstepb in reversed(range(params["eplen"])):
        ...
        LOSSMULT = (
            params["testlmult"]
            if numstepb > params["eplen"] - params["triallen"] * params["nbtesttrials"]
            else 1.0
        )
    ...
    loss += (
        params["lda"] * lossDA
    )  # lossDA is loss on absolute value of DA output (see above)
    loss += params["lhl1"] * lossHL1
    loss /= params["eplen"]
    losspw = (
        torch.mean(pw**2) * params["lpw"]
    ...
    if PRINTTRACE:
        print(
            "lossobj (with coeff):",
            lossobj / params["eplen"],
            ", lossv (with coeff): ",
            params["blossv"] * float(lossv) / params["eplen"],
            "lossDA (with coeff): ",
            params["lda"] * float(lossDA) / params["eplen"],
            ", losspw:",
            float(losspw),
        )
        print(
            ...
            sumreward[0],
            "Prop. of trials w/ rewarded cue:",
            (nbtrialswithcc / totalnbtrials),
            " Total Nb of trials:",
            totalnbtrials,
        )
        print(
        ...
        if nbtesttrials > 0:
            ...
            print(
                ...
                np.array([nbtesttrials_nonadjcues_correct / nbtesttrials_nonadjcues])
                if nbtesttrials_nonadjcues > 0
                else "N/A",
                "Test perf old cues:",
                np.array(
                    [
                        np.sum(iscorrect_thisep * istest_thisep * isolddata_thisep)
                        / np.sum(istest_thisep * isolddata_thisep)
                    ]
                )
                if np.sum(istest_thisep * isolddata_thisep) > 0
                else "N/A",
            )
    ...
    if PRINTTRACE:
        ...
        lossbetweensaves = 0
        print(
            "Mean reward per episode (over whole batch and last",
            params["pe"],
            "episodes: ",
            np.sum(all_mean_rewards_ep[-params["pe"] :]) / params["pe"],
        )
        print(
            "Mean test-time reward per episode (over whole batch and last",
            params["pe"],
            "episodes: ",
            np.sum(all_mean_testrewards_ep[-params["pe"] :]) / params["pe"],
        )
    ...
    if EVAL:
        np.savez(
            "outcomes_" + str(numepisode) + ".npz",
            c=iscorrect_thisep.astype(int),
            a=isadjacent_thisep.astype(int),
            cp=np.moveaxis(np.array(cuepairs_thisep), 1, 0),
            r=resps_thisep.astype(int),
            ac=numactionschosen_alltrialsandsteps_thisep.astype(int),
        )
...
if False:
    ...
    allpwsavs_thisep = [
        x * DAmultiplier if x is not None else None for x in allpwsavs_thisep
    ]
...
for numplot in range(4):
    ...
    if RESETHIDDENEVERYTRIAL:
        ...
        ds_blank01[0 :: params["triallen"]] = [
            0 for x in ds_blank01[0 :: params["triallen"]]
        ]
        ds_blank01[1 :: params["triallen"]] = [
            0 for x in ds_blank01[1 :: params["triallen"]]
        ]
    ...
    if NBMASSEDTRIALS > 0:
        plt.axvspan(
            params["nbtraintrials"] * TL,
            params["nbtraintrials"] * TL + NBMASSEDTRIALS * TL,
            color="orange",
            alpha=0.2,
            lw=0,
        )
...
if not LINKEDLISTSEVAL:
    ...
    rz = rs[
        numplot,
        NUMTRIAL_Z * params["triallen"] : NUMTRIAL_Z * params["triallen"]
        + params["triallen"] * 4
        + 1,
    ]
    dz = ds_blank01[
    ...
    maxlen = len(rz)
    plt.xticks(
        np.arange(0, maxlen, params["triallen"]),
        NUMTRIAL_Z + np.arange(0, maxlen // params["triallen"] + 1),
    )
    ...
    plt.plot(
        range(2, maxlen, params["triallen"]),
        dz[2 :: params["triallen"]],
        "ro",
        markersize=8,
        label="Step 3",
    )
    ...
    plt.plot(
        range(3, maxlen, params["triallen"]),
        dz[3 :: params["triallen"]],
        "rx",
        markersize=8,
        label="Step 4",
    )
...
if not LINKINGISSHAM:
    ...
    plt.violinplot(
        ...
        showextrema=False,
        showmedians=True,
        widths=[dt2_rneg.size / dt2.size, dt2_rpos.size / dt2.size],
    )
    plt.xlim((0.5, 2.5))
    ...
    plt.violinplot(
        ...
        showextrema=False,
        showmedians=True,
        widths=[dt3_rneg.size / dt3.size, dt3_rpos.size / dt3.size],
    )
    plt.ylim((-3.5, 3.5))
    plt.yticks((-3, 0, 3))
    plt.xlim((0.5, 2.5))
...
o = np.load("outcomes_" + str(e) + ".npz", allow_pickle=True)
c = o["c"]
a = o["a"]
na = 1 - a
cp = o["cp"]
        ...
        for nb in range(nsp * SPLITSIZE, nsp * SPLITSIZE + SPLITSIZE):
            for nt in range(cp.shape[1]):
                if nt < params["nbtrials"] - params["nbtesttrials"]:
                    ...
                    continue
                if (cp[nb, nt, 0] == p[0] and cp[nb, nt, 1] == p[1]) or (
                    cp[nb, nt, 0] == p[1] and cp[nb, nt, 1] == p[0]
                ):
                    nbthispair += 1
                    if (cp[nb, nt, 0] == p[0] and r[nb, nt] == 1) or (
                        cp[nb, nt, 0] == p[1] and r[nb, nt] == -1
...
]  # How did we live without StackOverflow?
...
for nump in range(params["nbcues"] - 1):
    ...
    plt.plot(
        range(strt + offset, strt + rnge + offset),
        np.median(allperfs[:, strt : strt + rnge], axis=0),
    )
    plt.fill_between(
        range(strt + offset, strt + rnge + offset),
        np.quantile(allperfs[:, strt : strt + rnge], 0.25, axis=0),
        np.quantile(allperfs[:, strt : strt + rnge], 0.75, axis=0),
        alpha=0.3,
    )
    ...
    if rnge == 1:
        ...
        plt.errorbar(
            [strt + offset],
            [
                np.median(allperfs[:, strt]),
            ],
            [
                [np.median(allperfs[:, strt]) - np.quantile(allperfs[:, strt], 0.25)],
                [np.quantile(allperfs[:, strt], 0.75) - np.median(allperfs[:, strt])],
            ],
        )  # The most troublesome line in the whole codebase. We love matplotlib.
...
if not FIXEDCUES:
    if LINKEDLISTSEVAL:
        plt.savefig(
            "SDE_LINKEDLISTS" + ("_SHAM" if LINKINGISSHAM else "") + ".png", dpi=300
        )
...
# To generate this figure, run the notebook with LINKEDLISTSEVAL=True twice:
# first with LINKINGISSHAM=False, then with LINKINGISSHAM=True. The linked
# result is persisted so the second pass also works in a fresh Python process.
if LINKEDLISTSEVAL:
    ...
    print(perfsperdist)
    if not LINKINGISSHAM:
        np.save(LINKED_PERFORMANCE_PATH, perfsperdist)
    else:
        if not LINKED_PERFORMANCE_PATH.is_file():
            raise FileNotFoundError(
                "Linked-list baseline is missing. Run once with "
                "LINKINGISSHAM=False before running the sham comparison: "
                f"{LINKED_PERFORMANCE_PATH}"
            )
        perfsperdist_link = np.load(LINKED_PERFORMANCE_PATH)
...
plt.plot(
    a[0, :10, -20:].T
)  # Notice the T. 10 first neurons, 20 last timesteps (for whole eplisode, so several trials)
...
print(
    cp[0, :, 0]
)  # For batch element 0, all trials, first of pair. They better correspondto the previous line!
...
for NUMTRIAL in range(params["nbtrials"]):
    model.fit(
        x[:-200, :, NUMTRIAL], yall[:-200, :, NUMTRIAL]
    )  # Last 200 are test set, all before that are train set
    ...
    for nc in range(yall.shape[1]):
        ...
        curves[NUMTRIAL, nc] = np.corrcoef(out.T[nc, :], yall[-200:, nc, NUMTRIAL].T)[
            0, 1
        ]
...
print(
    yall.shape
)  #  5000 (BBS * NbEpisodes), 5 (number of different quantities), 32 (NbTrials)
print(yall[:, 1, :].shape)
print(
    first_iscuenum.shape
)  # 8 (numbber of different cues),  5000 (BS * NbEpisodes), 32 (NbTrials)
...
plt.plot(
    mx_pca_firstis0[:, 0], mx_pca_firstis0[:, 1], ".g", alpha=0.3, label="First is 0"
)
mx_pca_firstis2 = mx_pca[first_iscuenum[2, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_firstis2[:, 0], mx_pca_firstis2[:, 1], ".r", alpha=0.3, label="First is 2"
)
mx_pca_firstis3 = mx_pca[first_iscuenum[3, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_firstis3[:, 0], mx_pca_firstis3[:, 1], ".b", alpha=0.3, label="First is 3"
)
mx_pca_firstis4 = mx_pca[first_iscuenum[4, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_firstis4[:, 0], mx_pca_firstis4[:, 1], ".y", alpha=0.3, label="First is 4"
)
mx_pca_firstis5 = mx_pca[first_iscuenum[5, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_firstis5[:, 0], mx_pca_firstis5[:, 1], ".c", alpha=0.3, label="First is 5"
)
plt.xlabel("PC 0")
plt.ylabel("PC 1")
plt.legend()
...
plt.plot(respPpoints[:, 0], respPpoints[:, 1], ".g", alpha=0.2, label="Resp+")

respNpoints = mx_pca[resps[:, NUMTRIALPCA] == -1, :]
plt.plot(respNpoints[:, 0], respNpoints[:, 1], ".r", alpha=0.2, label="Resp-")
plt.xlabel("PC 0")
plt.ylabel("PC 1")
plt.legend()
...
plt.plot(orderedpoints[:, 0], orderedpoints[:, 1], ".g", alpha=0.2, label="Ordered")

notorderedpoints = mx_pca[ordered[:, NUMTRIALPCA] == 0, :]
plt.plot(
    notorderedpoints[:, 0], notorderedpoints[:, 1], ".r", alpha=0.2, label="Not Ordered"
)
plt.xlabel("PC 0")
plt.ylabel("PC 1")
plt.legend()
...
plt.plot(
    prevcorrpoints[:, 0], prevcorrpoints[:, 1], ".g", alpha=0.2, label="PrevTrial Resp+"
)
...
plt.plot(
    prespPpoints[:, 2], prespPpoints[:, 3], ".g", alpha=0.2, label="Prev Trial Resp+"
)
prespNpoints = mx_pca[resps[:, NUMTRIALPCA - 1] == -1, :]
plt.plot(
    prespNpoints[:, 2], prespNpoints[:, 3], ".r", alpha=0.2, label="Prev Trial Resp-"
)
plt.xlabel("PCA 2")
plt.ylabel("PCA 3")
plt.legend()
...
plt.plot(
    preva0points[:, 0], preva0points[:, 1], ".g", alpha=0.2, label="PrevStep Action 0"
)
preva1points = mx_pca[ac[:, NUMTRIALPCA, POSINTRIALPCA - 1] == 1, :]
plt.plot(
    preva1points[:, 0], preva1points[:, 1], ".r", alpha=0.2, label="PrevStep Action 1"
)
plt.xlabel("PCA 0")
plt.ylabel("PCA 1")
plt.legend()
...
plt.plot(
    mx_pca_secondis0[:, 0], mx_pca_secondis0[:, 1], ".g", alpha=0.3, label="Second is 0"
)
mx_pca_secondis3 = mx_pca[second_iscuenum[3, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_secondis3[:, 0], mx_pca_secondis3[:, 1], ".b", alpha=0.3, label="Second is 3"
)
mx_pca_secondis2 = mx_pca[second_iscuenum[2, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_secondis2[:, 0], mx_pca_secondis2[:, 1], ".r", alpha=0.3, label="Second is 2"
)
mx_pca_secondis4 = mx_pca[second_iscuenum[4, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_secondis4[:, 0], mx_pca_secondis4[:, 1], ".y", alpha=0.3, label="Second is 4"
)
mx_pca_secondis5 = mx_pca[second_iscuenum[5, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_secondis5[:, 0], mx_pca_secondis5[:, 1], ".c", alpha=0.3, label="Second is 5"
)
plt.xlabel("PCA 0")
plt.ylabel("PCA 1")
plt.legend()
...
plt.subplot(3, 4, 9)
respPpoints = mx_pca[resps[:, NUMTRIALPCA] == 1, :]
plt.plot(respPpoints[:, 2], respPpoints[:, 3], ".g", alpha=0.2, label="Resp+")

respNpoints = mx_pca[resps[:, NUMTRIALPCA] == -1, :]
plt.plot(respNpoints[:, 2], respNpoints[:, 3], ".r", alpha=0.2, label="Resp-")
plt.xlabel("PCA 2")
plt.ylabel("PCA 3")
plt.legend()
...
mx_pca_secondis0 = mx_pca[second_iscuenum[0, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_secondis0[:, 2], mx_pca_secondis0[:, 3], ".g", alpha=0.3, label="Second is 0"
)
mx_pca_secondis3 = mx_pca[second_iscuenum[3, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_secondis3[:, 2], mx_pca_secondis3[:, 3], ".b", alpha=0.3, label="Second is 3"
)
mx_pca_secondis2 = mx_pca[second_iscuenum[2, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_secondis2[:, 2], mx_pca_secondis2[:, 3], ".r", alpha=0.3, label="Second is 2"
)
mx_pca_secondis4 = mx_pca[second_iscuenum[4, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_secondis4[:, 2], mx_pca_secondis4[:, 3], ".y", alpha=0.3, label="Second is 4"
)
mx_pca_secondis5 = mx_pca[second_iscuenum[5, :, NUMTRIALPCA] == 1, :]
plt.plot(
    mx_pca_secondis5[:, 2], mx_pca_secondis5[:, 3], ".c", alpha=0.3, label="Second is 5"
)
plt.xlabel("PCA 2")
plt.ylabel("PCA 3")
plt.legend()
...
respPpoints = mx_pca[resps[:, NUMTRIALPCA] == 1, :]
plt.plot(respPpoints[:, 0], respPpoints[:, 1], "+c", alpha=0.3, label="Choose Stim1")

respNpoints = mx_pca[resps[:, NUMTRIALPCA] == -1, :]
plt.plot(respNpoints[:, 0], respNpoints[:, 1], ".r", alpha=0.2, label="Choose Stim2")
plt.xlabel("PC 1")
plt.ylabel("PC 2")
...
plt.arrow(
    ...
    width=0.1,
    head_width=0.5,
)  # , label='$\mathbf{W}_{out}$')
# plt.text(-3.5, -.6, '$\mathbf{W}_{out}$', fontsize=15)
plt.text(0, -0.75, r"$\mathbf{W_{out}}$", fontsize=15)
...
plt.subplot(2, 2, 2)
orderedpoints = mx_pca[ordered[:, NUMTRIALPCA] == 1, :]
plt.plot(orderedpoints[:, 0], orderedpoints[:, 1], "+c", alpha=0.3, label="Stim1>Stim2")

notorderedpoints = mx_pca[ordered[:, NUMTRIALPCA] == 0, :]
plt.plot(
    notorderedpoints[:, 0], notorderedpoints[:, 1], ".r", alpha=0.2, label="Stim2>Stim1"
)
plt.xlabel("PC 1")
plt.ylabel("PC 2")
plt.legend()
...
currcorrpoints = mx_pca[corr[:, NUMTRIALPCA] == 1, :]
plt.plot(currcorrpoints[:, 0], currcorrpoints[:, 1], "+c", alpha=0.3, label="Correct")

currnotcorrpoints = mx_pca[corr[:, NUMTRIALPCA] == 0, :]
plt.plot(
    currnotcorrpoints[:, 0], currnotcorrpoints[:, 1], ".r", alpha=0.2, label="Error"
)
plt.xlabel("PC 1")
plt.ylabel("PC 2")
plt.legend()
...
mx_pca_firstis0 = mx_pca[first_iscuenum[0, :, NUMTRIALPCA] == 1, :]
plt.plot(mx_pca_firstis0[:, 0], mx_pca_firstis0[:, 1], ".g", alpha=0.3, label="Cue1:A")
mx_pca_firstis2 = mx_pca[first_iscuenum[2, :, NUMTRIALPCA] == 1, :]
plt.plot(mx_pca_firstis2[:, 0], mx_pca_firstis2[:, 1], ".r", alpha=0.3, label="Cue1:B")
mx_pca_firstis3 = mx_pca[first_iscuenum[3, :, NUMTRIALPCA] == 1, :]
plt.plot(mx_pca_firstis3[:, 0], mx_pca_firstis3[:, 1], ".b", alpha=0.3, label="Cue1:C")
mx_pca_firstis4 = mx_pca[first_iscuenum[4, :, NUMTRIALPCA] == 1, :]
plt.plot(mx_pca_firstis4[:, 0], mx_pca_firstis4[:, 1], ".y", alpha=0.3, label="Cue1:D")
# mx_pca_firstis5 = mx_pca[first_iscuenum[5, :, NUMTRIALPCA] == 1, :]
...
for nb in range(params["nbcues"]):
    ...
    inputs[nb, NBSTIMBITS + 0] = 1.0  # Bias neuron, probably not necessary
    inputs[nb, NBSTIMBITS + 1] = (
        0  # numstep_ep / params['eplen'] # Time passed in this episode. Should it be the trial? Doesn't matter much anyway.
    )
...
y, v, DAout, step1repressinglecues, et, pw = net(
    inputsN, hidden, et, pw
)  # y  should output raw scores, not probas
...
inputsN1[:, :NBSTIMBITS] = (
    0  # Normally, stimuli are only presented at the first time step
)
...
for nb in range(params["nbcues"]):
    print(
        "Cue", nb, ":", np.corrcoef(hiddenout[nb, :], wo)[0, 1]
    )  # , np.sum(hiddenout[nb,:]**2)  np.sum(hiddenout[nb,:] * wo) / np.sqrt(np.sum(wo ** 2)), np.sum(hiddenout[nb,:] * wo) / (np.sqrt(np.sum(wo ** 2)) * np.sqrt(sum(hiddenout[nb,:]**2))))
...
print(
    "Extracting  step-1 (feedforward) representations of all cues, for each batchelement (they  differ because each batchelement has its own randomly generated cues)"
)
...
for nb in range(params["nbcues"]):
    ...
    step1repressinglecuesallbatch.append(
        hidden.cpu().numpy()
    )  # This appends the batch of RNN states, i.e. the step-1 representations of cue nb for each batch element
...
allcorrs_s1 = []
print(
    "Mean (across batch) correlation of step-2 representation of each cue with output-weights vector\n(trial 20):"
)
...
for nb in range(params["nbcues"]):
    ...
    y, v, DAout, hidden, et, pw = net(
        inputsN0, hidden, et, pwtest
    )  # hidden (RNN state) is not the Step-1 (feedforward) representation (no effect of recurrence since RNN activations were reinitialized)
    y, v, DAout, hiddenout, et, pw = net(
        inputsN1, hidden, et, pw
    )  # hiddenout is now the Step-2 representations of the cue nb
    step2repressinglecuesallbatch.append(
        hiddenout.cpu().numpy()
    )  # This appends the batch of RNN states, i.e. the step-1 representations of cue nb for each batch element
    ...
    z = np.corrcoef(
        hiddenout, wo
    )  # Very wasteful, computes correlations of all the  hiddenouts in the batch with each other, rather than just b/w the hiddenouts and the wo! But so be it.
    z = z[:-1, -1]  # Last item is always 1.0

    zs1 = np.corrcoef(
        hidden.cpu().numpy(), wo
    )  # Step-1 representations, being linear projections of the random cue vectors, should not carry any information about rank
    zs1 = zs1[:-1, -1]
...
for nb in range(s1r.shape[1]):
    similarities_s1 = np.corrcoef(
        s1r[:, nb, :]
    )  # Matrix of correlations between  step-1 representations of all cues (for batch element nb)
    similarities_s2 = np.corrcoef(
        s2r[:, nb, :]
    )  # Matrix of correlations between  step-2 representations of all cues (for batch element nb)
    similarities_s1 = np.triu(
        similarities_s1, 1
    )  # Only include the top right part (*excluding* the diagonal of all-1s !)
...
# %%
print(
    "Correlation between the step-2 (learned) representation of each individual cue  (taken at the end of the training period for each episode, i.e. 20 trials), and the output weight vector w_out"
)
print(
    "Number of items, and number of episodes over which correlation is computed (should be =batch size):"
)
print(
    "Note: These figures should NOT use HALFNOBARREDPAIRUNTILT18, since it (deliberately) increases error rates and affects representation learning."
)
...
for numplot, allcorrs in enumerate([allcorrs_s1, allcorrs_s2]):
    ...
    plt.errorbar(
        np.arange(8),
        np.mean(allcorrs, axis=1),
        yerr=np.std(allcorrs, axis=1),
        color="b",
        marker="o",
        linestyle="none",
    )
...
print(
    "Although it doesn't change the appearance much, you can use HALFNOBARREDPAIRUNTILT18 here to ensure you get at least 1000 data points."
)
...
for nb in range(MYBS):
    include = True
    if (
        selectpair[0] not in cp[nb, NUMTRIALCHNGCORR, :]
        or selectpair[1] not in cp[nb, NUMTRIALCHNGCORR, :]
    ):
        include = False
    else:
...
for nb in selects:
    include = True
    for nt in range(NUMTRIALCHNGCORR):  # going to NUMTRIALCHNGCORR - 1 incl.
        if cp[nb, nt, 0] in [ADDBARREDPAIR[0], ADDBARREDPAIR[1]] and cp[nb, nt, 1] in [
            ADDBARREDPAIR[0],
            ADDBARREDPAIR[1],
        ]:  # Any order would cause pairing.
            include = False
            break
    if include:
...
for numstep in range(4):
    ...
    for nbcue in range(8):
        ...
        inputs[:, NBSTIMBITS + 2] = (
            0  # 1.0 * reward[nb] # Reward from previous time step
        )
        ...
        if numstep > 0:
            print(
                ...
                np.mean(cor[myselects] - corrseachstep[numstep - 1][nbcue][myselects]),
            )
        else:
...
if False:
    ...
    plt.bar(
        list(range(8)),
        [np.median(x[myselects]) for x in step2corrdiffs],
        yerr=np.vstack(
            (
                [
                    np.quantile(x[myselects], 0.75) - np.median(x[myselects])
                    for x in step2corrdiffs
                ],
                [
                    np.median(x[myselects]) - np.quantile(x[myselects], 0.25)
                    for x in step2corrdiffs
                ],
            )
        ),
    ...
    plt.bar(
        list(range(8)),
        [np.median(x[myselects]) for x in step3corrdiffs],
        yerr=np.vstack(
            (
                [
                    np.quantile(x[myselects], 0.75) - np.median(x[myselects])
                    for x in step3corrdiffs
                ],
                [
                    np.median(x[myselects]) - np.quantile(x[myselects], 0.25)
                    for x in step3corrdiffs
...
print(
    ...
    len(myselects),
    "(use HALFNOBARREDPAIRUNTILT18 = True in main code to make it larger)",
)
...
for nump in range(4):
    ...
    plt.errorbar(
        np.arange(8),
        np.mean(corrseachstep[nump, :, myselects].T, axis=1),
        yerr=np.std(corrseachstep[nump, :, myselects].T, axis=1),
        color="b",
        marker="o",
        linestyle="none",
    )
...
for nump in range(4):
    ...
    if nump > 1:
        corrdiffs = (
            corrseachstep[nump, :, myselects].T
            - corrseachstep[nump - 1, :, myselects].T
        )
        # plt.title("Step 3")  # 1-counting in the paper
        plt.axhline(0, color="k")
        plt.bar(
            list(range(8)),
            np.mean(corrdiffs, axis=1),
            yerr=np.std(corrdiffs, axis=1),
            color="r",
            edgecolor="k",
            lw=1,
            width=1.0,
        ...
        plt.axis("off")
        if nump == 0:
            plt.text(-0.7, 0.1, "Change from\nprevious step", rotation=90)
        plt.text(0.15, 0.5, "   No\nchange")
...
plt.savefig(
    ...
    + str(BARREDPAIR[1])
    + (
        "_add" + str(ADDBARREDPAIR[0]) + str(ADDBARREDPAIR[1])
        if not SHOWALLSELECTS
        else ""
    )
    + ".png",
    dpi=300,
...
if False:
    ...
    pwdiff = (
        allpwsavs_thisep[NUMTRIALCHNGCORR * params["triallen"] + 3][NUMBECHNGCORR, :, :]
        - allpwsavs_thisep[NUMTRIALCHNGCORR * params["triallen"] + 2][
            NUMBECHNGCORR, :, :
        ]
    )
    ...
    for nc in range(8):
        ...
        print(
            "With alpha:",
            np.corrcoef(mats[nc].flatten(), pwdiffalpha.flatten())[0, 1],
            ", without alpha:",
            np.corrcoef(mats[nc].flatten(), pwdiffnotalpha.flatten())[0, 1],
        )

    for nc in range(8):
        print(
            "Projection of pwdiff over outer prod of  cue1 and wo (flattened, with alpha):",
            np.sum(mats[nc].flatten() * pwdiffalpha.flatten())
            / np.linalg.norm(mats[nc].flatten()),
        )
    ...
    print(
        np.corrcoef(
            (-0.5 * mats[2] - mats[3] + mats[4] + 0.5 * mats[5]).flatten(),
            pwdiffalpha.flatten(),
        )[0, 1]
    )
...
if False:
    for nc in range(8):
        mat = np.matmul(
            pca.components_[0, :][:, None], step1repressinglecues[nc, :][None, :]
        )
        print(
            "With alpha:",
            np.corrcoef(mat.flatten(), pwdiffalpha.flatten())[0, 1],
            ", without alpha:",
            np.corrcoef(mat.flatten(), pwdiffnotalpha.flatten())[0, 1],
        )
...
if False:
    ...
    if True:
        ...
        for nc in range(8):
            ...
            v1s[nc].data = 0.01 * (v1s[nc].data - 0.5)

        v2 = torch.rand(HS, requires_grad=True)
        v2.data = 0.01 * (v2.data - 0.5)
        ...
        for numstep in range(3000):
            ...
            for nc in range(8):
                ...
                outera = 0.3 * ww.detach() + outer * alph.detach()
...
if False:
    ...
    print(
        "Correlations between the  found v1's (estimates of 'optimal' step-1 representations to produce learning)"
    )
    print(
        allrates.shape,
        v1s[0].shape,
        np.vstack(v1s).shape,
        np.corrcoef(np.vstack(v1s)).shape,
    )
    ...
    for nc in range(8):
        ...
        print(
            np.corrcoef(
                v1s[nc].detach(), step1repressinglecuesallbatch[nc][NUMBECHNGCORR, :]
            )[0, 1]
        )
    ...
    print(
        "Correlation  between found v1s and the H vector at timestep",
        ZS,
        " for BE",
        MYNUM,
        ":",
    ...
    print(
        "Correlation  between found v1s and the H vector at timestep 0 for BE",
        MYNUM,
        "(should be ~0 because it's trial step 0):",
    )
    for nc in range(8):
        print(np.corrcoef(v1s[nc].detach(), allrates[MYNUM, :, 0])[0, 1])
    print(
        "Correlation  between found v1s and the H vector at timestep 1 for BE",
        MYNUM,
        "(should be high+ for 1st cue of trial 0  and high- for 2nd cue of trial 0 -or vice versa):",
    )
    for nc in range(8):
        print(np.corrcoef(v1s[nc].detach(), allrates[MYNUM, :, 1])[0, 1])
    print(
        "Correlation  between found v1s and the H vector at timestep 5 for BE",
        MYNUM,
        "(should be high+ for 1st cue of trial 1  and high- for 2nd cue of trial 1 -or vice versa):",
    )
    for nc in range(8):
        print(np.corrcoef(v1s[nc].detach(), allrates[MYNUM, :, 5])[0, 1])
    print("==")
    print(
        "Correlation  between found v2 and the H vector at timesteps",
        ZS - 1,
        "to",
        ZS + 2,
        "(incl) (BE",
...
if True:
    ...
    v2 = torch.rand(
        HS, 1, requires_grad=True, device=device
    )  # No bbatch dimension !  1 common  v2 for the whole batch!So no worries about signs.
    v2.data = 0.01 * (v2.data - 0.5)
    ...
    for numstep in range(1000):
        ...
        for nc in range(8):
            ...
            outer = torch.matmul(
                v2[None, :, :], v1s[nc]
            )  # Outer has shape 2000, 200, 200 - as expected (batched outer product)
            outera = (
                1.0 * ww.detach()[None, :, :] + outer * alph.detach()[None, :, :]
            )  # outera:   2000, 200, 200
            prod = torch.matmul(
                outera, c1ps[nc].detach()[:, :, None]
            )  #  prrod has shape 2000, 200, 1

            cc = torch.nn.functional.cosine_similarity(
                prod, wwo[None, :, None].detach()
            )  # cc should have shape  2000,1
    ...
    torch.set_grad_enabled(False)
    print(
        "The value above should have reached -8000 or below. If that isn't the case, start again. (See comments in this cell)"
    )
...
sims = torch.nn.functional.cosine_similarity(
    tmp1, tmp2
).numpy()  # 2000. We use cosine_similarity because it can be  done in a batch
...
for nc in range(8):
    print(
        np.corrcoef(v1sN[nc][MYNUM, 0, :], step1repressinglecuesallbatch[nc][MYNUM, :])[
            0, 1
        ]
    )
...
pp(
    "selected in ss1 (trial",
    NUMTRIALCHNGCORR,
    " shows pair 3,4):",
    np.sum(ss1),
    "| selected in ss2 same trial shows pair 4,3):",
    np.sum(ss2),
)
...
for numstep in range(ZS - 1, ZS + 3):
    ...
    for nc in range(8):
        tmp1 = torch.from_numpy(
            v1sN[nc][:, 0, :]
        )  # 2000, 200.  The 0 is a dummy dimension
        tmp1b = torch.from_numpy(step1repressinglecuesallbatchN[nc, :, :])  # 2000, 200.
        tmp2 = torch.from_numpy(allrates[:, :, numstep])
        allsims_adap_thisstep.append(
            torch.nn.functional.cosine_similarity(tmp1, tmp2).numpy()
        )  # 2000. We use cosine_similarity because it can be  done in a batch
        allsims_orig_thisstep.append(
            torch.nn.functional.cosine_similarity(tmp1b, tmp2).numpy()
        )  # 2000. We use cosine_similarity because it can be  done in a batch
...
for numfig in range(2):
    ...
    for nump in range(4):
        ...
        plt.xticks(np.arange(8), alphabet[:8])
        plt.ylim([-0.35, 0.35] if numfig == 0 else [-0.8, 0.8])
        plt.errorbar(
            np.arange(8),
            np.mean(quantitytoplot[nump, :, ss1].T, axis=1),
            yerr=np.std(quantitytoplot[nump, :, ss1].T, axis=1),
            color="orange",
            ecolor="b",
            markerfacecolor="b",
            markeredgecolor="b",
        ...
        plt.errorbar(
            np.arange(8),
            np.mean(quantitytoplot[nump, :, ss2].T, axis=1),
            yerr=np.std(quantitytoplot[nump, :, ss2].T, axis=1),
            color="m",
            linestyle="--",
            ecolor="c",
            markerfacecolor="c",
        ...
        if nump == 0:
            plt.ylabel(
                r"Corr($\tilde{\psi}_{t1}(X), \mathbf{r}(t)$)"
                if numfig == 0
                else r"Corr($\psi_{t1}(X), \mathbf{r}(t)$)"
            )
        else:
            plt.yticks([])
    ...
    plt.tight_layout()
    plt.savefig(
        ("adapted" if numfig == 0 else "original") + "FFcuereps_t18_DEonly.png", dpi=300
    )
...
for numstep in range(4):
    meancorrs1.append(
        np.mean(np.corrcoef(allrates[ss1, :, ZS - 1 + numstep], v2N[:, 0])[-1, :-1])
    )
    stdcorrs1.append(
        np.std(np.corrcoef(allrates[ss1, :, ZS - 1 + numstep], v2N[:, 0])[-1, :-1])
    )
    meancorrs2.append(
        np.mean(np.corrcoef(allrates[ss2, :, ZS - 1 + numstep], v2N[:, 0])[-1, :-1])
    )
    stdcorrs2.append(
        np.std(np.corrcoef(allrates[ss2, :, ZS - 1 + numstep], v2N[:, 0])[-1, :-1])
    )
print(meancorrs1)
print(meancorrs2)
...
for nump in range(4):
    ...
    plt.errorbar(
        np.arange(8),
        np.mean(allsimsT1[nump, :, ss1].T, axis=1),
        yerr=np.std(allsimsT1[nump, :, ss1].T, axis=1),
        color="orange",
        ecolor="b",
        markerfacecolor="b",
        markeredgecolor="b",
    ...
    plt.errorbar(
        np.arange(8),
        np.mean(allsimsT1[nump, :, ss2].T, axis=1),
        yerr=np.std(allsimsT1[nump, :, ss2].T, axis=1),
        color="m",
        linestyle="--",
        ecolor="c",
        markerfacecolor="c",
...
for pair in zepairs:
    meansims_thispair = []
    ss = (cp[:, NUMTRIALCHNGCORR, 0] == pair[0]) & (
        cp[:, NUMTRIALCHNGCORR, 1] == pair[1]
    )
    for nc in range(8):
        tmp1 = torch.from_numpy(
            v1sN[nc][:, 0, :]
...
plt.yticks(
    range(8), [r"$\tilde{\psi}_{t1}($" + str(alphabet[x]) + r"$)$" for x in range(8)]
)
...
pp(
    corrseachstep.shape
)  # 4, 8, 2000: at each step, corrs of the step-1 (2nd step, after one pass through learned plastic weights) representation of each  cue with w_out (trial 18)
...
plt.subplot(1, 2, 1)
plt.plot(
    (allsims_adap[1, zecue, ss]),
    corrseachstep[3, zecue, ss] - corrseachstep[2, zecue, ss],
    ".",
)
...
plt.plot(
    (allsims_adap[1, zecue, ss1]) * resps[ss1, 18],
    (corrseachstep[3, zecue, ss1] - corrseachstep[2, zecue, ss1]),
    "c+",
    label="Correct",
)
plt.plot(
    (allsims_adap[1, zecue, ss2]) * resps[ss2, 18],
    (corrseachstep[3, zecue, ss2] - corrseachstep[2, zecue, ss2]),
    "r.",
    label="Incorrect",
)
...
print(
    cp.shape, a.shape
)  # (5000, 35, 2) (5000, 200, 70) (70 = nbtrials * nbstepspertrial)
