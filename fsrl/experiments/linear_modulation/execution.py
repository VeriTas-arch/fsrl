...
from fsrl.experiments.duplicate_observation.training import train_steps
...
from .model import common_hashes, make_model
...
def sources():
    ...
    paths += list((REPO_ROOT / "tests/experiments/linear_modulation").glob("*.py"))
...
def freeze():
    ...
    for ref in files + list(inherited.values()) + list(anchor_files.values()):
...
def validate_source():
    ...
    for ref in list(lock["parent_artifacts"].values()) + list(
        lock["anchor_artifacts"].values()
    ):
        verify_reference(ref, commit=lock["source_commit"])
...
def train():
    ...
    audit = load_json(RECORDS / "results/reference_audit.json")
    if audit["source_lock"] != reference(SOURCE) or not audit["passed"]:
        raise RuntimeError("frozen-model audit must complete before new training")
...
def check_reference_training(seed, arm, row, previous, source):
    metadata = row["metadata"]
    if common_hashes(metadata["initial_backbone"]) != common_hashes(
        previous["metadata"]["initial_backbone"]
    ):
        raise RuntimeError("common initialization differs from reference")
    if metadata["stream_fingerprint"] != previous["metadata"]["stream_fingerprint"]:
        raise RuntimeError("training task stream mismatch")
    original = source["parent_artifacts"][f"training/{seed}/{arm}/train_log.jsonl"]
    old_rows = [
        json.loads(line) for line in verify_reference(original).read_text().splitlines()
    ]
    new_rows = [
        json.loads(line)
    ...
    for first, second in zip(old_rows, new_rows, strict=True):
        if any(first[k] != second[k] for k in keys):
            raise RuntimeError("paired realized training inputs differ")


def lock_models():
    source = validate_source()
    prior, runs = parents()["models"]["runs"], {}
    for seed in specification()["design"]["seeds"]:
        ...
        for key in ("initial_backbone", "initial_local", "stream_fingerprint"):
            if pair["clean"]["metadata"][key] != pair["noisy"]["metadata"][key]:
                raise RuntimeError("A/C paired initialization or stream mismatch")
        for arm, row in pair.items():
            check_reference_training(seed, arm, row, prior[f"{seed}/{arm}"], source)
