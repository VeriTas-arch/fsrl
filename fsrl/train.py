...
def train(config, output_dir, trace_steps=False):
    ...
    for episode_index in episode_iter:
        ...
        loss_values.append(stats.loss_value)
    ...
    plt.plot(loss_values, linewidth=0.8)
    plt.xlabel("Episode")
    plt.ylabel("Loss")
    ...
    if len(loss_values) > 50:
        import pandas as pd
        smoothed = pd.Series(loss_values).rolling(window=50).mean()
        plt.plot(smoothed, color='orange', linewidth=2, label="smoothed (window=50)")
        plt.legend()

    loss_plot_path = output_dir / "loss_curve.png"
    plt.savefig(loss_plot_path, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"[save] Loss curve saved to {loss_plot_path}")
