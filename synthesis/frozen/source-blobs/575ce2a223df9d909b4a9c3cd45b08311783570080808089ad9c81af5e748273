    ...
    with np.load(trajectory_path) as artifact:
        for seed in (2101, 2102, 2103):
            ...
            for index, length_value in enumerate(lengths):
                ...
                used = (potentials[index, : length + 1], fields[index, : length + 1], evidence[index, :length])
                checks["valid_slices_finite"] &= all(np.all(np.isfinite(value)) for value in used)
...
def evaluate_fold(
    ...
    *,
    ...
    states, evidence, history, targets, episode_index = flatten_history_transitions(records)
    ...
    remote = {
        ...
        "frozen_v1_rank2_remote_mse": float(v1_metrics["remote_reassembly"]["candidate_remote_mse"]),
    }
    scalar = {
...
def evaluate_preservation(
    ...
    for relation in relations:
        ...
        global_loo.append(scalar_history_rollout_batch(loo_evidence, parameters) @ geometry.incidence.T)
        local_rows = []
        for subject, schedule in enumerate(evaluator.support_schedules):
            values = []
            for trial_index, trial in enumerate(schedule):
                ...
                values.append(
                    ...
                    * float(v1.access_factor(np.asarray([admission]), np.asarray([probability]))[0])
                )
            local_rows.append(
    ...
    positions = {item: index for index, item in enumerate(protocol.true_order_high_to_low)}
    ...
    exact = {
        "intact_nonlearned": v1._exact_accuracy(intact, true_sign, temperature, nonlearned),
        "global_only_nonlearned": v1._exact_accuracy(global_field, true_sign, temperature, nonlearned),
        "local_only_learned": v1._exact_accuracy(gain * local_raw, true_sign, temperature, learned),
        "local_only_nonlearned": v1._exact_accuracy(gain * local_raw, true_sign, temperature, nonlearned),
    }
    remote = {
        "intact": v1._remote_magnitude(intact, intact_loo, relations, geometry),
        "global_only": v1._remote_magnitude(global_field, global_loo, relations, geometry),
        "local_only": v1._remote_magnitude(gain * local_raw, local_loo, relations, geometry),
    }
    ...
    return {
        ...
        "identity_kernel_mean_abs_difference": float(np.mean(np.abs(local_raw - identity_raw))),
    }
