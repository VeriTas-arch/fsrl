"""Prospective paired Liu cohorts for the fresh replication."""
...
def lock_liu_inputs() -> dict:
    ...
    if not generic["summary"]["generic_gate_passed"]:
        raise RuntimeError(
...
def validate_input_lock() -> dict:
    ...
    if (
        ...
        or lock["selected_scheduler"] != "global"
        or lock["cohorts"] != COHORTS
        or lock["subjects"] != COHORT_SIZE
        or lock["model_rollout_performed"]
    ):
        raise RuntimeError("replication Liu input lock differs")
    ...
    if (
        ...
        raise RuntimeError("replication Liu input manifest differs")
    if len(lock["cohort_shards"]) * COHORT_SHARD_SIZE != COHORTS:
        raise RuntimeError("replication Liu input lock omits mandatory cohorts")
...
def _models(artifacts: dict) -> tuple[dict, dict]:
    ...
    for seed in SEEDS:
        for condition in CONDITIONS:
            ...
            models[identity] = load_model(
                artifacts["archives"][identity]["config"], spec, scheduler=scheduler
            )
...
def validate_output(record: dict, input_ref: dict, start: int) -> dict:
    if (
        ...
        raise RuntimeError("replication Liu output shard identity differs")
...
def evaluate_cohorts() -> dict:
    ...
    models, runners = _models(artifacts)
    execution = runtime()
    ...
    return {"cohorts": COHORTS, "fits": list(models), "selected_scheduler": "global"}
...
def summarize_points(points: list[dict]) -> dict:
    if [row["cohort"] for row in points] != list(range(COHORTS)):
        raise RuntimeError("replication summary requires every prospective cohort")
