...
DEFAULT_SPECIFICATION_PATH = resolve_record(
    "benchmarks/policy_opposition_gate_pilot_v2_1.json"
)
DEFAULT_LOCK_PATH = resolve_record(
    "benchmarks/policy_opposition_gate_pilot_v2_1.lock_v3.json"
)
DEFAULT_OUTPUT_ROOT = ROOT / "artifacts" / "runs" / "policy-opposition-gate-pilot-v2-1"
...
def decision_summary(
    ...
    contrasts = {
        "opposition_minus_original_local": summarize_difference(
            opposition["subject_level"][key],
            original["subject_level"][key],
            local["counts"],
            interval=local["interval"],
        ),
        "opposition_minus_original_H_greater_A": summarize_difference(
            opposition["subject_level"][h_key],
            original["subject_level"][h_key],
            local["counts"],
            interval=local["interval"],
        ),
        "opposition_minus_original_other_relations": summarize_difference(
    ...
    for control in (
        ...
        contrasts[f"opposition_minus_{control}_local"] = summarize_difference(
            opposition["subject_level"][key],
...
def evaluate_pilot(
    ...
    calibration_backbone, calibration_config, _ = load_frozen_retro_checkpoint(
