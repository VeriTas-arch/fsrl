"""Prospective source, fit and input barriers for the replication."""
...
def validate_qualification(record: dict) -> None:
    ...
    if any(record.get(key) != value for key, value in expected.items()):
        raise RuntimeError("global-decay replication qualification identity differs")
    if not record["checks"] or not all(
        row["passed"] is True for row in record["checks"].values()
    ):
        raise RuntimeError("global-decay replication qualification is incomplete")
...
def lock_source(qualification_directory) -> dict:
    ...
    if record["source_commit"] != commit:
        raise RuntimeError("qualify the final committed replication implementation")
...
def validate_source() -> dict:
    ...
    if (
        ...
        raise RuntimeError("global-decay replication source lock differs")
    ...
    if (
        ...
        raise RuntimeError("replication generic input manifest differs")
...
def validate_training(seed: int, condition: str) -> dict:
    ...
    if any(config.get(key) != value for key, value in expected.items()):
        raise RuntimeError("replication training identity differs")
    logs = [
        json.loads(line)
        for line in (directory / "train_log.jsonl").read_text().splitlines()
    ]
    if [row["step"] for row in logs] != list(range(settings["total_steps"])):
        raise RuntimeError("replication training log omits a registered step")
    for channel in ("base", "uniform", "encoded"):
        ...
        for row in logs:
            ...
            if row[f"{channel}_stream_sha256"] != digest.hexdigest():
                raise RuntimeError("replication training stream hash chain differs")
        if config[f"{channel}_stream_sha256"] != digest.hexdigest():
            raise RuntimeError("final replication training stream hash differs")
    ...
    model = make_model(condition, resolved_specification(), scheduler=scheduler)
    ...
    if tensor_hashes(model) != config["final_parameters"]:
        raise RuntimeError("archived replication parameters differ")
...
def _paired(runs: dict) -> None:
    ...
    if set(runs) != expected:
        raise RuntimeError("all six replication fits are mandatory")
    for seed in SEEDS:
        ...
        for key in (
            ...
            if rows[0][key] != rows[1][key]:
                raise RuntimeError(
                    "paired replication streams or initialization differ"
                )
...
def validate_artifacts() -> dict:
    ...
    if (
        ...
        raise RuntimeError("replication artifact/source lock differs")
    runs = {
    ...
    for identity, row in runs.items():
        ...
        if row != validate_training(int(seed), condition):
            raise RuntimeError("archived replication training record differs")
