"""Prospective source and content locks for the two-phase successor."""
...
def sources() -> list[dict]:
    paths = list(
        (REPO_ROOT / "fsrl/experiments/single_p_time_role_direct").glob("*.py")
    )
    paths += list(
        (REPO_ROOT / "tests/experiments/single_p_time_role_direct").glob("*.py")
    )
    paths += [
...
def _clean_commit() -> str:
    if subprocess.check_output(["git", "status", "--porcelain"], cwd=REPO_ROOT):
        raise RuntimeError("commit qualified direct-authority source before locking")
...
def write_source_lock(runtime: dict) -> dict:
    ...
    if not qualification["passed"] or qualification["sources"] != sources():
        raise RuntimeError("qualification does not cover committed direct source")
...
def validate_source_lock(runtime: dict) -> dict:
    ...
    if lock["protocol_sha256"] != PROTOCOL_SHA256 or lock["runtime"] != runtime:
        raise RuntimeError("direct-authority source lock differs")
...
def write_baseline_artifact_lock() -> dict:
    ...
    if not paths:
        raise RuntimeError("direct baseline has no runtime artifacts")
    payload = {
        "schema_version": 1,
        "protocol_sha256": PROTOCOL_SHA256,
...
def validate_baseline_artifact_lock() -> dict:
    ...
    if result["outcome"] != "baseline_compatible":
        raise RuntimeError("frozen direct baseline is not compatible")
