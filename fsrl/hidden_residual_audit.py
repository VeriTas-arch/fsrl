...
def _hidden_and_margin_fields(
    ...
    margins = np.empty(
        (evaluator.config.bs, len(geometry.pairs)), dtype=np.float64
    )
...
def _summarize_seed(
    *,
    ...
    omitted_selector = np.broadcast_to((~retained)[..., None, None], full_influence.shape)
    omitted_max = (
    ...
    diagnosis = {
        "causal_local_residual": (
            summaries["direct_minus_remote_residual_norm"]["bootstrap"]["lower"]
            > 0.0
            and omitted_max <= tolerance
        ),
        "linearly_accessible_local_direction": (
            summaries["local_direct_correctness"]["bootstrap"]["lower"] > 0.0
            and summaries["local_direct_minus_remote_absolute"]["bootstrap"][
                "lower"
            ]
            > 0.0
        ),
        "fixed_readout_suppression": (
            summaries["local_minus_output_direct_correctness"]["bootstrap"][
                "lower"
            ]
            > 0.0
        ),
