"""
Paper-aligned behavioral evaluation for single_neo_mutants_distance_input_v3 checkpoints.
...
Critical safety/interpretation choices:
  1. Test reward is always forced to zero and labels are never fed back.
  2. The primary paper-aligned cue mode reuses one shared set of eight item
     vectors across subjects, while independently permuting item-to-rank mapping.
  3. The paper's exact high-accuracy exclusion is implemented as accuracy > .5
     on every one of the 28 pairs, rather than a tie-broken majority ranking.
  4. Pairwise 5/5 ties are not counted as correct under the default strict tie
     policy; the selected policy is recorded in the output.
  5. Phase-rescaled time preserves the training learning/test boundary while
     mapping the much longer paper schedule into the model's trained time range.
  6. Stochastic testing samples from the model's own softmax policy with an
     explicit temperature; no fitted behavioral noise is injected.
"""
...
SCRIPT_VERSION = "simple_neo_paper_behavior_eval_2026-07-10_paper_aligned_v4"
...
try:
    ...
    import single_neo_mutants_distance_input_v3_checked as neo  # type: ignore[no-redef]  # noqa: E402
...
def make_distance_observation(config: neo.TrainConfig, cue_pairs: List[Pair], nbcues: int, is_train_trial: bool) -> np.ndarray:
    ...
    Distance is supplied only when neo.should_provide_distance_input says the
    current phase may expose it. With the default train-only setting this means
    behavioral learning trials get distance and paper/no-feedback test trials get zero.
    """
...
def build_step_inputs_eval(
    ...
    elif time_mode == "phase_rescale":
        train_learning_steps = config.nbtraintrials * config.triallen
        train_test_steps = config.nbtesttrials * config.triallen
        train_boundary = train_learning_steps / max(1, config.eplen)
        train_last = (config.eplen - 1) / max(1, config.eplen)
        if numstep_ep < eval_learning_steps:
            progress = numstep_ep / max(1, eval_learning_steps)
            time_value = progress * train_boundary
        else:
            eval_test_steps = max(1, eval_total_steps - eval_learning_steps)
            progress = (numstep_ep - eval_learning_steps) / eval_test_steps
            time_value = train_boundary + progress * (train_last - train_boundary)
        time_value = min(max(time_value, 0.0), train_last)
    else:
...
def choose_action_and_probs(y_raw: torch.Tensor, action_mode: str, choice_temperature: float) -> Tuple[torch.Tensor, torch.Tensor]:
    """Return actions and policy probabilities used for evaluation.

    Temperature is an explicit readout parameter. It does not change the model,
    learning input, labels, plasticity, or rewards. A temperature of 1.0 uses the
    model's native softmax scale; lower values make choices more deterministic,
    higher values read out more uncertainty from the same logits.
    """
    if choice_temperature <= 0:
        raise ValueError("--choice-temperature must be > 0")
    probs = torch.softmax(y_raw / float(choice_temperature), dim=1)
    if action_mode == "greedy":
        # Greedy keeps the deterministic decision boundary. We still report the
        # probability under the explicitly selected temperature for diagnostics.
        return torch.argmax(y_raw, dim=1), probs
...
def run_one_trial(
    ...
    for numstep in range(config.triallen):
        ...
        if numstep == neo.NUMRESPONSESTEP:
            actions_t, probs_t = choose_action_and_probs(y_raw, action_mode, choice_temperature)
            actions = actions_t.detach().cpu().numpy().astype("int32")
...
def make_paper_cue_data(
    config: neo.TrainConfig,
    nbcues: int,
    cue_set_mode: str,
    rng: np.random.Generator,
    shared_base_cues: Optional[List[np.ndarray]],
):
    """Create cue vectors under either the original or paper stimulus regime."""
    if cue_set_mode == "independent_random":
        return neo.generate_cue_data(config, nbcues)
    if cue_set_mode != "paper_shared_permuted":
        raise ValueError(f"Unknown cue_set_mode: {cue_set_mode}")
    if shared_base_cues is None or len(shared_base_cues) != nbcues:
        raise ValueError("paper_shared_permuted requires one shared base cue set")

    cue_data = []
    for _ in range(config.bs):
        permutation = rng.permutation(nbcues)
        cue_data.append([
            np.asarray(shared_base_cues[int(permutation[rank_index])], dtype=np.float32).copy()
            for rank_index in range(nbcues)
        ])
    return cue_data
...
def orient_pair_for_learning(config: neo.TrainConfig, pair: Pair, rng: np.random.Generator) -> Pair:
    ...
    if neo.is_passive_observational_variant(config.variant):
        # No response/feedback during learning. In distance-input v3 the signed
        # distance itself carries direction, so randomize orientation to avoid
        # a first-item shortcut. If distance input is disabled, preserve v2 behavior.
        random_orientation = (
...
def majority_winners_and_y(
    choice_counts_for_i: np.ndarray,
    total_counts: np.ndarray,
    tie_policy: str,
) -> Tuple[Dict[Pair, int], np.ndarray, int]:
    """Build pairwise majority judgments and the continuous Hodge preference matrix.

    For i<j, Y[i,j] is the mean preference for i over j in [-1, 1]. The
    preregistered high-accuracy exclusion is computed separately from raw pair
    accuracies and therefore never depends on this tie policy.
    """
    ...
    for i, j in all_unordered_pairs(N_ITEMS):
        ...
        if c_i > c_j:
            ...
            ties += 1
            if tie_policy == "incorrect":
                winners[(i, j)] = j
            elif tie_policy == "true":
                winners[(i, j)] = i
            else:
                raise ValueError(f"Unknown tie_policy: {tie_policy}")
    return winners, y_mat, ties

def classify_beta_params(alpha: float, beta: float) -> str:
...
def fit_beta_distribution(values: np.ndarray) -> Tuple[Optional[float], Optional[float], str]:
    """Fit/classify a Beta distribution for pair-level subject accuracies.

    The paper classifies cross-subject pair accuracy profiles via alpha/beta.
    This implementation does not require SciPy: if SciPy is unavailable, or MLE
    fails, it uses a method-of-moments estimate after epsilon clipping. Exact 0/1
    accuracies from deterministic policies are therefore not silently discarded.
    """
    ...
    def mom_estimate(vals: np.ndarray):
        m = float(np.mean(vals))
        v = float(np.var(vals, ddof=1)) if len(vals) > 1 else 0.0
        if v <= 1e-12:
            if m >= 0.95:
                return 9999.0, 0.5, "high_accuracy"
            if m <= 0.05:
                return 0.5, 9999.0, "low_accuracy"
            return 9999.0, 9999.0, "unimodal"
        max_v = m * (1.0 - m)
        if max_v <= 0:
            return None, None, "not_fit"
        # Sampling variance cannot exceed Bernoulli variance. If it does due to
        # boundary-heavy samples, cap slightly below the maximum; this yields
        # alpha,beta < 1, i.e. a U-shaped/bimodal profile when appropriate.
        v = min(v, max_v * (1.0 - 1e-6))
        common = max_v / v - 1.0
        alpha = m * common
        beta = (1.0 - m) * common
        if not np.isfinite(alpha) or not np.isfinite(beta) or alpha <= 0 or beta <= 0:
            return None, None, "not_fit"
        return float(alpha), float(beta), classify_beta_params(alpha, beta)

    if stats is not None:
        try:
            alpha, beta, _loc, _scale = stats.beta.fit(clipped, floc=0, fscale=1)
            if np.isfinite(alpha) and np.isfinite(beta) and alpha > 0 and beta > 0:
                return float(alpha), float(beta), classify_beta_params(float(alpha), float(beta))
        except Exception:
            pass
    return mom_estimate(clipped)
...
def evaluate(config: neo.TrainConfig, net: neo.RetroModulRNN, args: argparse.Namespace):
    ...
    eval_learning_steps = learning_len * config.triallen
    eval_total_steps = (learning_len + test_len) * config.triallen

    shared_base_cues: Optional[List[np.ndarray]] = None
    if args.cue_set_mode == "paper_shared_permuted":
        shared_base_cues = []
        for cue_index in range(N_ITEMS):
            shared_base_cues.append(
                np.asarray(neo.sample_unique_cue(config, shared_base_cues, cue_index), dtype=np.float32)
            )
    ...
    with torch.no_grad():
        for _batch_idx in range(n_batches):
            ...
            nbcues = N_ITEMS
            cue_data = make_paper_cue_data(config, nbcues, args.cue_set_mode, rng, shared_base_cues)
            hidden = net.initialZeroState(config.bs)
            et = net.initialZeroET(config.bs)
            ...
            for b in range(config.bs):
                ...
                winners, y_mat, tie_count = majority_winners_and_y(choose_i_counts[b], total_counts[b], args.tie_policy)
                ...
                # Paper's exact pair-level exclusion criterion: accuracy > .5
                # for every one of the 28 tested pairs.
                pair_acc_array = np.asarray(pair_acc_vec, dtype=np.float64)
                paper_high_accuracy_all_pairs = bool(
                    np.all(np.isfinite(pair_acc_array)) and np.all(pair_acc_array > 0.5)
                )
                if paper_high_accuracy_all_pairs:
                ...
                subject_rows.append(
                    {
                        ...
                        "paper_high_accuracy_all_pairs": int(paper_high_accuracy_all_pairs),
    ...
    # Paper analysis pipeline: first retain the final behavioral cohort, then
    # exclude subjects whose accuracy exceeds .5 on every pair for pair-level
    # beta/error/idiosyncrasy analyses.
    overall_values = np.asarray([r["overall_accuracy"] for r in subject_rows], dtype=np.float64)
    if args.paper_exclude_below_chance:
        eligible_mask = overall_values >= 0.5
    else:
        eligible_mask = np.ones(len(subject_rows), dtype=bool)
    correct_mask = np.asarray(
        [bool(r["paper_high_accuracy_all_pairs"]) for r in subject_rows], dtype=bool
    )
    if args.paper_exclude_correct_rankers:
        analysis_mask = eligible_mask & ~correct_mask
    else:
        analysis_mask = eligible_mask.copy()
    ...
    for pair_idx, (i, j) in enumerate(full_pairs):
        vals_generated = pair_acc_by_subject[:, pair_idx]
        vals_group = pair_acc_by_subject[eligible_mask, pair_idx]
        ...
        row_common = {
            ...
            "mean_accuracy_generated_subjects": float(np.nanmean(vals_generated)),
            "mean_accuracy_eligible_group": float(np.nanmean(vals_group)) if vals_group.size else np.nan,
            "mean_accuracy_analysis_subjects": float(np.nanmean(vals_analysis)) if vals_analysis.size else np.nan,
            "n_subjects_generated": int(len(vals_generated)),
            "n_subjects_eligible_group": int(np.sum(eligible_mask)),
    ...
    def count_class(cls: str, mask: Optional[np.ndarray] = None):
        vals = np.asarray([r["ranking_class"] == cls for r in subject_rows], dtype=bool)
        if mask is not None:
            vals = vals[mask]
    ...
    def mean_error_pair_fraction(threshold_label: int, mask: Optional[np.ndarray] = None):
        vals = np.asarray(
            [r[f"n_consistent_error_pairs_{threshold_label}"] / n_pairs for r in subject_rows],
            dtype=np.float64,
        )
        if mask is not None:
            vals = vals[mask]
    ...
    distance_mean = np.nanmean(distance_acc_by_subject[eligible_mask], axis=0)
    position_mean = np.nanmean(position_acc_by_subject[eligible_mask], axis=0)
    slope_stats = slope_and_t(distance_acc_by_subject[eligible_mask])
    ...
    summary = {
        ...
        "eval_subjects_generated": int(len(subject_rows)),
        "eligible_group_subjects": int(np.sum(eligible_mask)),
        "excluded_below_chance_subjects": int(np.sum(~eligible_mask)),
        "excluded_high_accuracy_all_pairs_subjects": int(np.sum(eligible_mask & correct_mask)),
        "analysis_subjects_after_filters": int(np.sum(analysis_mask)),
        "paper_exclude_below_chance": bool(args.paper_exclude_below_chance),
        ...
        "cue_set_mode": args.cue_set_mode,
        ...
        "overall_accuracy": mean_col("overall_accuracy", eligible_mask),
        "learned_pairs_accuracy": mean_col("learned_accuracy", eligible_mask),
        "nonlearned_pairs_accuracy": mean_col("nonlearned_accuracy", eligible_mask),
        "adjacent_accuracy": mean_col("adjacent_accuracy", eligible_mask),
        "nonadjacent_accuracy": mean_col("nonadjacent_accuracy", eligible_mask),
        "mean_prob_correct": mean_col("mean_prob_correct", eligible_mask),
        ...
        "mean_circular_triads": mean_col("circular_triads", eligible_mask),
        "mean_circular_triads_analysis": mean_col("circular_triads", analysis_mask),
        "mean_self_consistency_coefficient": mean_col("self_consistency_coefficient", eligible_mask),
        "mean_self_consistency_coefficient_analysis": mean_col("self_consistency_coefficient", analysis_mask),
        "mean_kendall_tau_hodge_to_true": mean_col("kendall_tau_hodge_to_true", eligible_mask),
        "mean_kendall_tau_hodge_to_true_analysis": mean_col("kendall_tau_hodge_to_true", analysis_mask),
        "ranking_class_counts": {
            "correct": count_class("correct", eligible_mask),
            "self_consistent_incorrect": count_class("self_consistent_incorrect", eligible_mask),
            "self_inconsistent": count_class("self_inconsistent", eligible_mask),
        },
        "ranking_class_counts_generated": {
        ...
        "mean_fraction_consistent_error_pairs_analysis": {},
        ...
        "mean_abs_pw_after_learning": mean_col("mean_abs_pw_after_learning", eligible_mask),
        "mean_abs_pw_after_test": mean_col("mean_abs_pw_after_test", eligible_mask),
    }
    for label in [60, 70, 80, 90, 100]:
        n_all, prop_all = has_count(label, eligible_mask)
        ...
        summary["mean_fraction_consistent_error_pairs_analysis"][str(label)] = mean_error_pair_fraction(label, analysis_mask)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Paper-aligned behavioral evaluator for single_neo_mutants_distance_input_v3 checkpoints.")
    ...
    parser.add_argument("--action-mode", choices=["greedy", "sample"], default="greedy", help="greedy gives deterministic upper-bound choices; sample draws from the model's own softmax policy.")
    parser.add_argument("--choice-temperature", type=float, default=1.0, help="Softmax temperature used for policy probabilities and stochastic sampling. T=1 uses native logits; lower is more deterministic; higher is more stochastic.")
    parser.add_argument("--time-mode", choices=["phase_rescale", "clamp", "original", "rescale_paper_eval"], default="phase_rescale")
    parser.add_argument("--freeze-test-plastic", action="store_true", help="Keep plastic weights fixed during no-feedback test blocks.")
    parser.add_argument("--cue-set-mode", choices=["paper_shared_permuted", "independent_random"], default="paper_shared_permuted", help="Paper mode reuses one shared item set and randomly permutes its rank assignment per subject.")
    parser.add_argument("--tie-policy", choices=["incorrect", "true"], default="incorrect", help="How to binarize an exact 5/5 pairwise tie for circular-triad analysis. Strict paper-aligned default does not count a tie as correct.")
    parser.add_argument("--paper-exclude-below-chance", action="store_true", help="Apply the paper's initial cohort exclusion: overall test accuracy below 50 percent.")
    parser.add_argument("--paper-exclude-correct-rankers", action="store_true", help="For pair-level analyses, exclude subjects with accuracy strictly above 50 percent on all 28 pairs.")
...
def main() -> None:
    ...
    log(f"[setup] test_orientation={args.test_orientation}; action_mode={args.action_mode}; choice_temperature={args.choice_temperature}; time_mode={args.time_mode}; freeze_test_plastic={args.freeze_test_plastic}")
    log(f"[setup] cue_set_mode={args.cue_set_mode}; tie_policy={args.tie_policy}; exclude_below_chance={args.paper_exclude_below_chance}; exclude_high_accuracy={args.paper_exclude_correct_rankers}")
