"""Matched final-checkpoint joint training, without an unused single-arm trace."""
...
def train_steps(backbone, local, sequence, spec, seed, directory) -> dict:
    ...
    with (directory / "train_log.jsonl").open("x") as log:
        for step in range(spec["optimization"]["total_steps"]):
            ...
            if step % 100 == 0 or step == spec["optimization"]["total_steps"] - 1:
                log.flush()
                print(
                    json.dumps({"seed": seed, "local": local is not None, **row}),
                    flush=True,
                )
...
def train_one(spec: dict, source: dict, seed: int, condition: str):
    ...
    with ProspectiveRun.start(
        ...
        stats = train_steps(backbone, local, sequence, spec, seed, directory)
