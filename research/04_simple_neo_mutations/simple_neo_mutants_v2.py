...
ADDINPUT = 4
NUMRESPONSESTEP = 1
SCRIPT_VERSION = "simple_neo_mutants_v2_2026-07-09"
...
def prepare_trial(config: TrainConfig, nbcues: int, numtrial: int):
    ...
    for batch_index in range(config.bs):
        if config.variant in ("exact_simple_neo", "n8_fixed"):
            ...
            if is_train_trial:
                # Passive demonstration: observed sparse relation, oriented correct-first by default.
                # There is no required response and no reward/feedback re-entered during learning.
                # If we add an observed-pair supervised auxiliary loss, use random
                # orientation so the auxiliary label cannot collapse into a trivial
                # "always choose the first displayed item" rule. Without train_aux,
                # correct-first ordering is the only passive demonstration signal.
                train_random_orientation = (
                    use_train_aux_loss(config.variant)
