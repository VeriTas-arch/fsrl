...
SCREEN = RECORDS / "benchmarks/screen_lock.json"
...
def sources():
    ...
    paths += list(
        (REPO_ROOT / "tests/experiments/observation_uncertainty").glob("*.py")
    )
...
def lock_development():
    validate_source()
    spec = specification()
    jobs = [(spec["seeds"]["development"], arm) for arm in spec["seeds"]["conditions"]]
    result = {
        "runs": paired_records(jobs),
        "files": [reference(SOURCE)],
        "status": "three_development_models_locked_before_validation",
    }
...
def lock_models():
    ...
    result = {
        "runs": paired_records(jobs),
        "files": [reference(SCREEN)],
        "status": "nine_final_models_locked_before_primary_outcomes",
    }
