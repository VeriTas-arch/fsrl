...
IMPLEMENTATION_LOCK_PATH = resolve_record(
    "benchmarks/dual_state_reduced_algorithm_v1.repair3.lock.json"
)
TRAJECTORY_PATH = resolve_record(
    "results/dual_state_reduced_algorithm_v1.trajectories.npz"
)
...
def reduced_step(
    ...
    if parameters.U is not None:
        delta = (
            delta
            + ((states @ parameters.V) * (evidence @ parameters.W)) @ parameters.U.T
        )
...
def fit_rank2_candidate(
    ...
    *,
    ...
    U = torch.nn.Parameter(
        torch.randn(8, 2, generator=generator, dtype=dtype, device=device) * 0.01
    )
...
def flatten_transitions(records: list[EpisodeTrajectory]) -> tuple[np.ndarray, ...]:
    ...
    episode_index = np.concatenate(
        [
            np.full(len(record.evidence), index, dtype=np.int64)
...
def rollout(
    evidence: np.ndarray, parameters: ReducedParameters, initial=None
) -> np.ndarray:
...
def evaluate_development_fold(
    ...
    *,
    ...
    for record in records:
        ...
        teacher_norm = np.linalg.norm(record.potentials[-1]) * np.linalg.norm(
            teacher_rollout[-1]
        )
        teacher_terminal_cosines.append(
            ...
            if teacher_norm <= 1e-12
                ...
                np.dot(record.potentials[-1], teacher_rollout[-1]) / teacher_norm
            )
        )
        ...
        terminal_agreement.append(
            float(
                np.mean(
                    np.sign(full_terminal_field) == np.sign(candidate_terminal_field)
                )
            )
        )
    ...
    metrics = {
        ...
        "trajectory": {
            ...
            "mean_teacher_initialized_terminal_cosine": float(
                np.mean(teacher_terminal_cosines)
            ),
        ...
        "remote_reassembly": {
            ...
            "candidate_remote_mse": float(
                np.mean((candidate_remote - full_remote) ** 2)
            ),
        ...
        "local_exact_max_abs_error": float(
            max(record.local_exact_error for record in records)
        ),
    }
...
def _evaluator_field(
    evaluator: FrozenFastWeightEvaluator, fast_weights: torch.Tensor, geometry: Geometry
) -> np.ndarray:
    ordered = tuple(
        oriented
        for first, second in geometry.pairs
...
def _liu_evidence(evaluator: FrozenFastWeightEvaluator) -> np.ndarray:
    values = np.zeros(
        (evaluator.config.bs, evaluator.protocol.support_trials, 8), dtype=np.float64
    )
...
def _human_intervals() -> tuple[dict, dict, dict, dict]:
    map_spec = load_json(
        resolve_record("benchmarks/model_behavior_reproduction_map_v1.json")
    )
    map_result = load_json(
...
def _exact_accuracy(
    field: np.ndarray, true_sign: np.ndarray, temperature: float, mask: np.ndarray
) -> float:
...
def evaluate_preservation_seed(
    ...
    for relation in relations:
        ...
        global_loo.append(
            rollout_batch(loo_evidence, parameters) @ geometry.incidence.T
        )
        local_rows = []
        for subject, schedule in enumerate(evaluator.support_schedules):
            values = []
    ...
    true_sign = np.asarray(
        [
            1.0 if true_positions[i] < true_positions[j] else -1.0
            for i, j in geometry.pairs
        ]
    )
    ...
    exact = {
        ...
        "intact_nonlearned": _exact_accuracy(
            intact, true_sign, temperature, nonlearned
        ),
        "global_only_nonlearned": _exact_accuracy(
            global_field, true_sign, temperature, nonlearned
        ),
        "local_only_learned": _exact_accuracy(
            gain * local_raw, true_sign, temperature, learned
        ),
        "local_only_nonlearned": _exact_accuracy(
        ...
        value.update(
            {
                ...
                "V": parameters.V.tolist(),
                "W": parameters.W.tolist(),
            }
        )
    return value
...
def build_result(
    ...
    for seed in specification["development_artifacts"]["mandatory_seeds"]:
        ...
        records = extract_development_seed(
            ...
            rng_seed=int(
                specification["generic_trajectory_contract"]["episode_rng_seeds"][
                    str(seed)
                ]
            ),
            batches=int(
                specification["generic_trajectory_contract"]["batches_per_backbone"]
            ),
            geometry=geometry,
        )
    ...
    for held_out in mandatory:
        train = [
            ...
            if seed != held_out
            for record in development[str(seed)]
        ]
    ...
    integrity = {
        ...
        "episodes_per_development_seed": all(
            len(records) == 128 for records in development.values()
        ),
