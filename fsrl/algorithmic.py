...
from .behavioral import hodge_rank_positions, kendall_tau_positions
...
def compare_neural_policy_to_exact_posterior(
    ...
    *,
    ...
    exact = ExactRankingPosterior(
        protocol.n_items, temperature=posterior_temperature
    )
    order_to_index = {
    ...
    for subject_index, (evidence_rows, logits) in enumerate(
        ...
        for first, second in pairs:
            margin = 0.5 * (
                logits[(first, second)] - logits[(second, first)]
            )
    ...
    return {
        ...
        "group": {
            ...
            "mean_neural_posterior_probability": mean(
                "neural_posterior_probability"
            ),
        },
        "subjects": rows,
    }
