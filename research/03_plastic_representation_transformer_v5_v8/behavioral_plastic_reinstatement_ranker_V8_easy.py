"""
V8 feedback-curriculum behavioral plastic-reinstatement ranker.
...
# Smaller fixed sparse graphs for temporary difficulty-reduction experiments.
# They preserve the broad paper format: a fixed sparse set of observed rank pairs,
# multiple learning blocks, and no-feedback all-pair evaluation. They are not meant
# to replace the original n=8 paper graph.
EASY_LEARNING_PAIRS_BY_N = {
    4: [(0, 2), (1, 3), (0, 3), (1, 2)],
    5: [(0, 3), (0, 4), (1, 2), (1, 4), (2, 3)],
    6: [(0, 3), (0, 5), (1, 2), (1, 4), (2, 5), (3, 4)],
}


def canonical_learning_pairs_rank(n_items: int, n_learning_pairs: int) -> list[tuple[int, int]]:
    if n_items == 8 and n_learning_pairs == 8:
        return list(PAPER_LEARNING_PAIRS_RANK)
    if n_items in EASY_LEARNING_PAIRS_BY_N:
        base = list(EASY_LEARNING_PAIRS_BY_N[n_items])
    else:
        # Generic fallback: start from a connected cycle, then add longer chords.
        base = []
        seen = set()
        def add(a: int, b: int) -> None:
            ...
            if x != y and (x, y) not in seen:
                seen.add((x, y))
                base.append((x, y))
        # Prefer non-adjacent chords first where possible.
        step = max(2, n_items // 2)
        for i in range(n_items):
            add(i, (i + step) % n_items)
        # Ensure connectivity with a light adjacent cycle fallback.
        for i in range(n_items):
            add(i, (i + 1) % n_items)
    max_pairs = n_items * (n_items - 1) // 2
    n_learning_pairs = min(max(1, int(n_learning_pairs)), max_pairs)
    if len(base) >= n_learning_pairs:
        return base[:n_learning_pairs]
...
class Config:
    ...
    pe: int = 50
    save_every: int = 1000
    output_dir: str = "outputs_behavioral_plastic_v8"
    device: str = "auto"
    amp: bool = False
    ...
    # feedback curriculum
    curriculum_mode: str = "adaptive"  # adaptive|constant|none
    fixed_feedback_prob: float = 1.0
    stage1_feedback_prob: float = 1.0
    stage2_feedback_floor: float = 0.0
    stage3_feedback_prob: float = 0.0
    stage1_min_episodes: int = 1000
    stage2_min_episodes: int = 2000
    stage1_acc_threshold: float = 0.82
    stage2_acc_threshold: float = 0.78
    curriculum_ema_decay: float = 0.98
    curriculum_decay_interval: int = 200
    feedback_prob_decay: float = 0.05
    feedback_prob_recovery: float = 0.02
    feedback_signal: str = "reward"  # reward|soft_reward|target_sign|signed_error|none
    eval_feedback_prob: float = 0.0
...
class PhaseStats:
    ...
    feedback_fraction: torch.Tensor
    mean_abs_feedback: torch.Tensor
...
class CurriculumState:
    ...
    stage_start_ep: int = 0
    last_decay_ep: int = -1


def initial_curriculum_state(config: Config) -> CurriculumState:
    if config.curriculum_mode == "none":
        return CurriculumState(stage="zero", feedback_prob=0.0)
    if config.curriculum_mode == "constant":
        return CurriculumState(stage="constant", feedback_prob=float(config.fixed_feedback_prob))
    return CurriculumState(stage="stage1_feedback_rich", feedback_prob=float(config.stage1_feedback_prob))


def update_curriculum_state(config: Config, state: CurriculumState, episode: int, acc: float) -> CurriculumState:
    if config.curriculum_mode in {"none", "constant"}:
        return state
    if state.ema_acc is None:
        state.ema_acc = float(acc)
    else:
        d = config.curriculum_ema_decay
        state.ema_acc = d * state.ema_acc + (1.0 - d) * float(acc)

    episodes_in_stage = episode - state.stage_start_ep + 1
    if state.stage == "stage1_feedback_rich":
        if episodes_in_stage >= config.stage1_min_episodes and state.ema_acc >= config.stage1_acc_threshold:
            state.stage = "stage2_feedback_dropout"
            state.stage_start_ep = episode + 1
            state.last_decay_ep = episode
            state.feedback_prob = min(state.feedback_prob, config.stage1_feedback_prob)
        return state

    if state.stage == "stage2_feedback_dropout":
        can_adjust = (state.last_decay_ep < 0) or (episode - state.last_decay_ep >= config.curriculum_decay_interval)
        if can_adjust:
            if state.ema_acc >= config.stage2_acc_threshold:
                state.feedback_prob = max(config.stage2_feedback_floor, state.feedback_prob - config.feedback_prob_decay)
                state.last_decay_ep = episode
            elif config.feedback_prob_recovery > 0:
                # Conservative recovery: if the network collapses after dropout, restore a little feedback.
                state.feedback_prob = min(config.stage1_feedback_prob, state.feedback_prob + config.feedback_prob_recovery)
                state.last_decay_ep = episode
        if episodes_in_stage >= config.stage2_min_episodes and state.feedback_prob <= config.stage3_feedback_prob + 1e-8:
            state.stage = "stage3_zero_feedback"
            state.stage_start_ep = episode + 1
            state.feedback_prob = float(config.stage3_feedback_prob)
        return state

    if state.stage == "stage3_zero_feedback":
        state.feedback_prob = float(config.stage3_feedback_prob)
        return state
    return state
...
def sample_sparse_rank_graph(config: Config) -> list[tuple[int, int]]:
    if config.paper_graph_train_prob > 0 and random.random() < config.paper_graph_train_prob:
        return canonical_learning_pairs_rank(config.n_items, config.n_learning_pairs)
...
class PlasticReinstatementRanker(nn.Module):
    def __init__(self, config: Config):
        ...
        # Relation features are split into observed relation/magnitude and
        # an explicit external feedback channel. In zero-feedback episodes the
        # relation can still be observed, but feedback is clamped to zero.
        self.rel_feat_dim = 7
    ...
    def relation_features(self, relation: torch.Tensor, phase: str, confidence: Optional[torch.Tensor] = None,
                          feedback_signal: Optional[torch.Tensor] = None) -> torch.Tensor:
        cfg = self.config
        if confidence is None:
            confidence = torch.ones_like(relation)
        if feedback_signal is None:
            feedback_signal = torch.zeros_like(relation)
        ...
        return torch.stack([r, mag, feedback_signal, confidence, is_test, is_replay, torch.ones_like(relation)], dim=1)
    ...
    def pair_context(self, codes: torch.Tensor, state: State, subject_z: torch.Tensor, item_i: torch.Tensor, item_j: torch.Tensor,
                     relation: torch.Tensor, phase: str, confidence: Optional[torch.Tensor] = None,
                     feedback_signal: Optional[torch.Tensor] = None) -> torch.Tensor:
        bidx = torch.arange(codes.shape[0], device=DEVICE)
        psi = self.transform_all_items(codes, state, subject_z)
        pi = psi[bidx, item_i]
        pj = psi[bidx, item_j]
        rel = self.relation_features(relation, phase, confidence, feedback_signal)
    ...
    def observe_pair(self, codes: torch.Tensor, state: State, subject_z: torch.Tensor, item_i: torch.Tensor, item_j: torch.Tensor,
                     relation: torch.Tensor, phase: str, eta_scale: float, confidence: Optional[torch.Tensor] = None,
                     feedback_signal: Optional[torch.Tensor] = None) -> tuple[State, torch.Tensor, torch.Tensor]:
        context = self.pair_context(codes, state, subject_z, item_i, item_j, relation, phase, confidence, feedback_signal)
    ...
    def reinstate(self, codes: torch.Tensor, state: State, subject_z: torch.Tensor, steps: int, eta_scale: float) -> tuple[State, list[torch.Tensor], list[torch.Tensor]]:
        ...
        for _ in range(steps):
            ...
            rel = self.relation_features(zero_rel, phase="replay", confidence=torch.zeros_like(zero_rel), feedback_signal=torch.zeros_like(zero_rel))
...
def run_learning_phase(config: Config, net: PlasticReinstatementRanker, subject_z: torch.Tensor, true_scores: torch.Tensor,
                       learning_edges: torch.Tensor, train_mode: bool, feedback_prob: float = 0.0) -> tuple[State, torch.Tensor, PhaseStats]:
    """Run the behaviorally grounded learning phase.

    V8 uses a staged feedback curriculum on top of the predict -> observed feedback -> plastic update loop. For each
    ...
    observed_buffer: list[tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]] = []
    ...
    bidx = torch.arange(bsz, device=DEVICE)
    if feedback_prob <= 0:
        feedback_available = torch.zeros(bsz, device=DEVICE)
    elif feedback_prob >= 1:
        feedback_available = torch.ones(bsz, device=DEVICE)
    else:
        feedback_available = (torch.rand(bsz, device=DEVICE) < feedback_prob).float()
    feedback_vals: list[torch.Tensor] = []

    def predict_observed_pair(item_i: torch.Tensor, item_j: torch.Tensor, relation: torch.Tensor, scale: float) -> torch.Tensor:
        ...
        if config.lambda_learning_aux <= 0 and config.lambda_axis_margin <= 0:
            return logit
        valid = (relation.abs() > 1e-7).float()
        label = (relation > 0).float()
        if config.lambda_learning_aux > 0:
            bce = F.binary_cross_entropy_with_logits(logit, label, reduction="none")
            aux_losses.append(scale * masked_mean(bce, valid))
            pred = (torch.sigmoid(logit) >= 0.5).float()
            aux_accs.append(masked_mean((pred == label).float(), valid))
        if config.lambda_axis_margin > 0:
            signed_logit = logit * (label * 2.0 - 1.0)
            margin_losses.append(scale * masked_mean(F.relu(config.learning_margin - signed_logit), valid))
            observed_signed_logits.append(signed_logit.detach()[valid.bool()] if valid.bool().any() else signed_logit.detach().reshape(0))
        return logit

    def compute_feedback_signal(logit: torch.Tensor, relation: torch.Tensor, feedback_mask: torch.Tensor) -> torch.Tensor:
        valid = (relation.abs() > 1e-7).float()
        if config.feedback_signal == "none":
            raw = torch.zeros_like(relation)
        else:
            label = (relation > 0).float()
            target_sign = label * 2.0 - 1.0
            logit_detached = logit.detach().float()
            if config.feedback_signal == "target_sign":
                # More informative scaffold; still restricted to shown learning pairs.
                raw = target_sign
            elif config.feedback_signal == "soft_reward":
                # Positive when the current prediction is aligned with the shown relation.
                raw = target_sign * torch.tanh(logit_detached)
            elif config.feedback_signal == "signed_error":
                raw = (target_sign - torch.tanh(logit_detached)).clamp(-1.0, 1.0)
            else:
                pred = (torch.sigmoid(logit_detached) >= 0.5).float()
                raw = torch.where(pred == label, torch.ones_like(relation), -torch.ones_like(relation))
        fb = (raw * feedback_mask * valid).clamp(-1.0, 1.0)
        feedback_vals.append(fb.abs().mean())
        return fb

    def feedback_update(item_i: torch.Tensor, item_j: torch.Tensor, relation: torch.Tensor, phase: str, eta_scale: float,
                        feedback_signal: torch.Tensor) -> None:
        """Reveal the shown relation and, if available, an external feedback channel."""
        nonlocal state
        state, mod, eta_abs = net.observe_pair(codes, state, subject_z, item_i, item_j, relation, phase=phase, eta_scale=eta_scale, feedback_signal=feedback_signal)
    ...
    for _block in range(config.n_learning_blocks):
        ...
        for k in range(n_edges):
            ...
            # Step 1: prediction before feedback. This prediction is used only for
            # shown learning pairs, never for unobserved all-pair labels.
            pre_logit = predict_observed_pair(item_i, item_j, rel, scale=1.0)

            # Step 2: relation is observed in all variants; the external feedback
            # channel is Bernoulli-gated by the staged curriculum.
            fb = compute_feedback_signal(pre_logit, rel, feedback_available)
            feedback_update(item_i, item_j, rel, phase="learn", eta_scale=1.0, feedback_signal=fb)
            observed_buffer.append((item_i.detach(), item_j.detach(), rel.detach(), feedback_available.detach()))
            ...
            if config.observed_replay_steps > 0 and observed_buffer:
                for _ in range(config.observed_replay_steps):
                    ...
                    ri, rj, rr, rfb_mask = observed_buffer[rb]
                    replay_logit = predict_observed_pair(ri, rj, rr, scale=config.replay_aux_scale)
                    replay_fb = compute_feedback_signal(replay_logit, rr, rfb_mask)
                    feedback_update(ri, rj, rr, phase="replay", eta_scale=config.observed_replay_eta_scale, feedback_signal=replay_fb)
    ...
    if observed_signed_logits:
        flat_logits = [x.reshape(-1) for x in observed_signed_logits if x.numel() > 0]
        observed_score_std = torch.cat(flat_logits).std(unbiased=False) if flat_logits else zero
    else:
    ...
    stats = PhaseStats(
        ...
        feedback_fraction=feedback_available.mean(),
        mean_abs_feedback=torch.stack(feedback_vals).mean() if feedback_vals else torch.zeros((), device=DEVICE),
...
def run_training_episode(config: Config, net: PlasticReinstatementRanker, feedback_prob: float = 0.0, curriculum_stage: str = "manual") -> EpisodeStats:
    bsz = config.bs
    ...
    state, codes, learn_stats = run_learning_phase(config, net, subject_z, true_scores, learning_edges, train_mode=True, feedback_prob=feedback_prob)
    ...
    return EpisodeStats(
        ...
        feedback_fraction=float(learn_stats.feedback_fraction.detach()),
...
def evaluate_paper_task(config: Config, net: PlasticReinstatementRanker) -> dict:
    ...
    true_scores = true_scores_by_item(r2i, config)
    eval_rank_edges = canonical_learning_pairs_rank(config.n_items, config.n_learning_pairs)
    learning_edges = rank_edges_to_item_edges(eval_rank_edges, r2i, random_orientation=False)
    state, codes, learn_stats = run_learning_phase(config, net, subject_z, true_scores, learning_edges, train_mode=False, feedback_prob=config.eval_feedback_prob)
    ...
    paper_pair_set = {tuple(p) for p in eval_rank_edges}
    ...
    summary = {
        ...
        "eval_feedback_prob": config.eval_feedback_prob,
        "model_type": "v8_feedback_curriculum_plastic_rank_axis_no_direct_item_writer",
        ...
        "learning_feedback_fraction": float(learn_stats.feedback_fraction.detach().cpu()),
        "learning_mean_abs_feedback": float(learn_stats.mean_abs_feedback.detach().cpu()),
...
def train(config: Config) -> PlasticReinstatementRanker:
    ...
    with open(out / "config_v8.json", "w") as f:
    ...
    log("[setup] V8: staged feedback curriculum + zero-feedback paper-task deployment + plastic representation transformer; no direct item-state writer")
    log(f"[setup] use_rnn={config.use_rnn}; use_hebbian={config.use_hebbian}; test_update_mode={config.test_update_mode}")
    log(f"[setup] bs={config.bs}; nbiter={config.nbiter}; n_items={config.n_items}; n_learning_pairs={config.n_learning_pairs}; item_code_dim={config.item_code_dim}; rnn_hidden={config.rnn_hidden_size}")
    log(f"[setup] fixed_learning_pairs_rank={canonical_learning_pairs_rank(config.n_items, config.n_learning_pairs)}")
    ...
    log(f"[setup] curriculum_mode={config.curriculum_mode}; fixed_feedback_prob={config.fixed_feedback_prob}; feedback_signal={config.feedback_signal}; eval_feedback_prob={config.eval_feedback_prob}")
    log(f"[setup] adaptive: stage1_min={config.stage1_min_episodes}, stage1_thr={config.stage1_acc_threshold}, stage2_min={config.stage2_min_episodes}, decay_interval={config.curriculum_decay_interval}, decay={config.feedback_prob_decay}")
    log(f"[setup] parameters={sum(p.numel() for p in net.parameters())}")
    fields = [
        "episode", "stage", "feedback_prob", "feedback_fraction", "loss", "choice_loss", "entropy_loss", "acc", "learned_acc", "nonlearned_acc",
    ...
    curriculum = initial_curriculum_state(config)
    iterator = tqdm(range(config.nbiter), desc="training episodes", unit="episode", dynamic_ncols=True, file=sys.stdout)
    for ep in iterator:
        opt.zero_grad(set_to_none=True)
        current_feedback_prob = float(curriculum.feedback_prob)
        current_stage = str(curriculum.stage)
        with torch.cuda.amp.autocast(enabled=use_amp):
            stats = run_training_episode(config, net, feedback_prob=current_feedback_prob, curriculum_stage=current_stage)
        ...
        scaler.update()
        last = stats
        curriculum = update_curriculum_state(config, curriculum, ep, stats.acc)
        with open(out / "train_log.csv", "a", newline="") as f:
            csv.writer(f).writerow([
                ep, stats.curriculum_stage, stats.feedback_prob, stats.feedback_fraction, stats.loss_value, stats.choice_loss, stats.entropy_loss, stats.acc, stats.learned_acc, stats.nonlearned_acc,
        ...
        if ep % config.pe == 0 or ep == config.nbiter - 1:
            ...
            log(
                f"Episode {ep} [{stats.curriculum_stage} p_fb={stats.feedback_prob:.3f} used={stats.feedback_fraction:.3f}] ==== {elapsed:.2f}s | "
                f"loss={stats.loss_value:.4f} choice={stats.choice_loss:.4f} "
        ...
        if config.save_every > 0 and ep > 0 and ep % config.save_every == 0:
            torch.save({"model_state": net.state_dict(), "config": asdict(config)}, out / "net_v8.pt")
            log(f"[save] checkpoint: {out / 'net_v8.pt'}")
    torch.save({"model_state": net.state_dict(), "config": asdict(config)}, out / "net_v8.pt")
    if last is not None:
...
def parse_args() -> Config:
    p = argparse.ArgumentParser(description="V8 behavioral plastic ranker: staged feedback curriculum, observed-pair replay, no direct item-state writer.")
    ...
    p.add_argument("--output-dir", type=str, default="outputs_behavioral_plastic_v8")
    ...
    p.add_argument("--n-learning-pairs", type=int, default=0,
                   help="Number of fixed sparse learning pairs. 0 means auto: n_items for n!=8, 8 for n=8.")
    ...
    p.add_argument("--curriculum-mode", choices=["adaptive", "constant", "none"], default="adaptive")
    p.add_argument("--fixed-feedback-prob", type=float, default=1.0)
    p.add_argument("--stage1-feedback-prob", type=float, default=1.0)
    p.add_argument("--stage2-feedback-floor", type=float, default=0.0)
    p.add_argument("--stage3-feedback-prob", type=float, default=0.0)
    p.add_argument("--stage1-min-episodes", type=int, default=1000)
    p.add_argument("--stage2-min-episodes", type=int, default=2000)
    p.add_argument("--stage1-acc-threshold", type=float, default=0.82)
    p.add_argument("--stage2-acc-threshold", type=float, default=0.78)
    p.add_argument("--curriculum-ema-decay", type=float, default=0.98)
    p.add_argument("--curriculum-decay-interval", type=int, default=200)
    p.add_argument("--feedback-prob-decay", type=float, default=0.05)
    p.add_argument("--feedback-prob-recovery", type=float, default=0.02)
    p.add_argument("--feedback-signal", choices=["reward", "soft_reward", "target_sign", "signed_error", "none"], default="reward")
    p.add_argument("--eval-feedback-prob", type=float, default=0.0)
    ...
    args = p.parse_args()
    if args.n_learning_pairs <= 0:
        args.n_learning_pairs = 8 if args.n_items == 8 else args.n_items
    if args.n_items < 3:
        raise ValueError("--n-items must be >= 3")
    max_pairs = args.n_items * (args.n_items - 1) // 2
    if args.n_learning_pairs > max_pairs:
        raise ValueError(f"--n-learning-pairs={args.n_learning_pairs} exceeds all possible pairs={max_pairs}")
