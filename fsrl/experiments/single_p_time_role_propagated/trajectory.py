...
def _probe_susceptibility(model, weight, codes):
    ...
    control_ordered, control_pairs = read_ordered(model, before, code_batch)
    control_fields = np.stack(
        [
            _canonical_field(row, pair_row)
            for row, pair_row in zip(control_ordered, control_pairs, strict=True)
        ]
    )
    ...
    functional = np.sqrt(np.mean((fields - control_fields) ** 2, axis=1))
