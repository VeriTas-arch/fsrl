...
def sources():
    ...
    paths += list((REPO_ROOT / "tests/experiments/local_memory_removal").glob("*.py"))
...
def freeze():
    ...
    for ref in files + list(inherited.values()):
...
def validate_source():
    ...
    for ref in lock["parent_artifacts"].values():
        verify_reference(ref, commit=lock["source_commit"])
        ...
        for arm, row in pair.items():
            ...
            for key in ("initial_backbone", "stream_fingerprint"):
                if row["metadata"][key] != previous["metadata"][key]:
                    raise RuntimeError("single and archived dual mismatch")
