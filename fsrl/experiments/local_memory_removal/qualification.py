"""Independent single-state reference and archived dual collector qualification."""
...
from .model import rollout, update
...
def numerical_checks(device="cpu", compiled=False):
    ...
    actual, _, _ = rollout(net, None, seqs, batch, None, 0)
...
def qualify():
    ...
    from .statistics import qualification_checks
    ...
    with ProspectiveRun.start(
        ...
        execution_id="optional-local-qualification",
        ...
        artifacts = prior["result"]["artifacts"]
        ...
        _, raw, _ = collect(2531, "clean", panel, model, recipe(1), 1002531, "dual")
        ...
        for phase in raw:
            for key, value in raw[phase].items():
                if not np.array_equal(value, old[phase][key], equal_nan=True):
                    raise RuntimeError(f"dual replay differs: {phase}/{key}")
