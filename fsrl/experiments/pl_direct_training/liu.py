"""Frozen Liu-v2 rollout for the paired direct-training models."""
...
from .optimization import DirectBackbone, run_sequence


class DirectLiuEvaluator:
    def __init__(
        self,
        condition: str,
        ...
        *,
        ...
        self.test_time_value = test_time_value
        self.item_rank = {
        ...
        self.sequence = compile_module(sequence, execution_profile)
    ...
    def _support_inputs(
        ...
        *,
        ...
        time_value = (
            trial_index / max(1, self.protocol.support_trials - 1)
        ) * self.test_time_value
        inputs, times = direct_input_arrays(
            self.cue_codes,
            pairs,
            evidence,
            np.asarray([time_value], dtype=np.float32),
            steps=4,
        )
        return (
            torch.from_numpy(inputs[0]).to(self.device),
            torch.from_numpy(times[0]).to(self.device),
        )
    ...
    def advance_support(
        ...
        *,
        ...
        inputs, times = self._support_inputs(
            trial_index,
    ...
    def query_bundle(
        ...
        *,
        ...
        inputs, times = direct_input_arrays(
            ...
            np.full(count, self.test_time_value, dtype=np.float32),
            steps=2,
        )
        ...
        times = torch.from_numpy(
            times.transpose(1, 0, 2, 3).reshape(2, count * self.subjects, 1).copy()
        ).to(self.device)
        ...
        with torch.no_grad(), self.alpha_zeroed(alpha_zero):
            ...
            gain_override = torch.zeros_like(global_margin) if local_off else None
            margins, raw, gain, correction = self.local(
                global_margin,
...
def rollout_liu(evaluator: DirectLiuEvaluator, settings: dict) -> dict:
    ...
    queries = shuffled_pair_indices(
        evaluator.subjects, protocol.n_items, settings["query_shuffle_seed"]
    )
    bundles = {
    ...
    if not np.array_equal(
        ...
        raise RuntimeError("direct-training local-off identity failed")
    loo = {"global": [], "local": [], "combined": []}
    for relation in relations:
        removed = frozenset((relation,))
        loo_weights = evaluator.learn_fast_weights(zero_relations=removed)
        loo_local, _, _ = evaluator.build_local_state(zero_relations=removed)
        loo["global"].append(
