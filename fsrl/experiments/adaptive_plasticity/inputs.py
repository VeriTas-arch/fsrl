"""Protected generic schedule inputs and prospective Liu cohort shards."""
...
from .data import clustered_batch
...
def save_generic_inputs() -> dict:
    ...
    for length, indices in sorted(groups.items()):
        ...
        clustered, clustered_uniforms, permutations = clustered_batch(
            batch, uniforms, cluster_rng
        )
        key = str(length)
        episode_indices = np.asarray(indices, dtype=np.int64)
        records["balanced"][key] = _write_batch(
            destination / f"generic-balanced-{length}.npz",
            batch,
            uniforms,
            episode_indices=episode_indices,
        )
        records["clustered"][key] = _write_batch(
            destination / f"generic-clustered-{length}.npz",
            clustered,
            clustered_uniforms,
            episode_indices=episode_indices,
            trial_permutations=permutations,
        )
    result = {
...
def load_group(record: dict) -> tuple[ModelBatch, dict[str, np.ndarray]]:
    ...
    if batch.fingerprint() != record["batch_sha256"]:
        raise RuntimeError("saved adaptive-plasticity batch changed")
    return batch, {
...
def cohort_indices(start: int) -> list[int]:
    if start not in range(0, COHORTS, COHORT_SHARD_SIZE):
        raise ValueError("unregistered Liu cohort shard")
