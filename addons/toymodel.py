...
for numplot, REINSTATE in enumerate([False, True]):
    ...
    for numepisode in range(NBEPISODES):
        ...
        for numtrial in range(20):
            ...
            if REINSTATE:
                if pos1 > 0:
                    vals[pos1 - 1] += 0.5 * incr
                if pos2 < 7:
                    vals[pos2 + 1] -= 0.5 * incr
    ...
    print(
        "Mean/std of corrz b/w vals and arange(8) (reinstate=" + str(REINSTATE) + "):",
        np.mean(corrz),
        np.std(corrz),
    )
    ...
    plt.fill_between(np.arange(8), meanz - stdz, meanz + stdz, color=mycolor, alpha=0.3)
...
print(
    "Mann-Whitney U-test (2-sided) on the groups of correlations from both plots (without/with reinstate):",
