...
def prepare_trial(config: TrainConfig, nbcues: int, numtrial: int):
    ...
    for batch_index in range(config.bs):
        ...
        elif config.variant in ("behavior_graph_rewarded", "behavior_graph_no_test_feedback"):
        ...
        elif config.variant == "observational_learning":
            if is_train_trial:
                # Passive demonstration: observed sparse relation, oriented correct-first.
                cue_pair = sample_behavior_graph_pair(random_orientation=not config.observational_train_orients_correct_first)
        ...
        if config.variant == "behavior_graph_no_test_feedback" and is_test_trial:
            reward_available[batch_index] = 0.0
    ...
    return cues, cue_pairs, correct_order, adjacent, requires_choice, reward_available
...
def run_episode(config: TrainConfig, net: RetroModulRNN, nbcues: int, print_trace: bool = False):
    ...
    for numtrial in range(config.nbtrials):
        ...
        cues, cue_pairs, correct_order, adjacent, requires_choice, reward_available = prepare_trial(
            config, nbcues, numtrial
        )
...
def log_trace(
    ...
    log(
        ...
        "TTHCC(0): {} Reward(prev): {} DAout: {} cues(0): {} "
...
def train(config: TrainConfig, output_dir: Path, trace_steps: bool = False):
    ...
    if config.variant in ("behavior_graph_rewarded", "behavior_graph_no_test_feedback", "observational_learning"):
...
def main():
    ...
    if args.variant == "n8_fixed":
        nmin = nmax = 8
    elif args.variant in ("behavior_graph_rewarded", "behavior_graph_no_test_feedback", "observational_learning"):
        nmin = nmax = 8
