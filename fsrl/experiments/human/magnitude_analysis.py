...
def _choice_probability(
    pair_probability: dict[str, float], pair: str, role: str
) -> float:
    first, second = pair.split("-")
    if role == first:
...
def profile_metrics(profile: dict, protocol: dict) -> dict:
    ...
    for condition in ("A", "B"):
        ...
        condition_metrics[condition] = {
            ...
            "same_direction_accuracy": float(np.mean([correct[pair] for pair in same])),
        }
        ranks[condition] = hodge_rank_positions(field)
    order_positions = {
        condition: np.asarray(
            [
                list(reversed(inherited[f"assignment_{condition}_low_to_high"])).index(
                    role
                )
                for role in ROLES
            ],
...
def profile_from_session_bundles(bundles: list[dict]) -> dict:
    ...
    for bundle in bundles:
        ...
        for trial in query:
            first, second = sorted(
                (trial["role_left"], trial["role_right"]), key=ROLES.index
            )
            chosen = (
                trial["role_left"]
                if trial["choice_side"] == "left"
                else trial["role_right"]
            )
            choices[f"{first}-{second}"].append(float(chosen == first))
        if set(choices) != set(PAIRS) or any(
            len(values) != 10 for values in choices.values()
        ):
...
def analyze_profiles(profiles: list[dict], protocol: dict) -> dict:
    ...
    return {
        ...
        "secondary": {
            "assignment_following_score": results["assignment_following_score"]
        },
        "descriptive": {
            "condition_metrics": condition_descriptives,
            "pair_choice_probability": pair_fields,
            "pair_difference_B_minus_A": {
                pair: pair_fields["B"][pair] - pair_fields["A"][pair] for pair in PAIRS
            },
            "hodge_orders": {
    ...
    for condition, positions in (("A", positions_a), ("B", positions_b)):
        for pair in PAIRS:
            _set_choice_probability(
                fields[condition], pair, _higher(pair, positions), 0.75
            )
    for pair in partition["nonlearned_order_flip"]:
        higher_a = _higher(pair, positions_a)
        probability_a = _choice_probability(fields["A"], pair, higher_a)
        _set_choice_probability(fields["B"], pair, higher_a, probability_a - delta_flip)
    for pair in partition["nonlearned_same_direction"]:
        first, second = pair.split("-")
        ...
        _set_choice_probability(
            ...
            pair,
            higher,
            _choice_probability(fields["A"], pair, higher) + beta_conf * delta_distance,
        )
    gaps = {
        condition: dict(
            zip(
...
def validate_all_synthetic_branches(protocol: dict | None = None) -> dict:
    ...
    gates = {
        expected: result["outcome"] == expected and all(result["gates"].values())
