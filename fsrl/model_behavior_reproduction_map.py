...
def _model_record(
    ...
    bootstrap_seed = int(specification["bootstrap"]["human_new_estimands_seed"]) + int(seed)
    profiles = {
...
def build_map(
    ...
    if not all(identity_gates.values()):
        raise RuntimeError(f"behavior reproduction map identity gate failed: {identity_gates}")
    ...
    human_serial["profiles"] = {
        name: endpoint_statistics(
            np.mean(
                np.asarray(
                    [record[name]["profile_low_to_high"] for record in human_serial_by_subject]
                ),
                axis=0,
            )
        )
    ...
    human_analysis = [
        ...
        if subject["ranking_class"] != "correct"
    ]
    ...
    if not np.isclose(
        human_tau["point"], human_benchmark["combined"]["mean_inter_subject_kendall_tau"]
    ):
