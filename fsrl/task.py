...
def build_step_inputs(
    config, nbcues, cue_data, cues, previous_actions, numstep, numstep_ep, is_train_trial
):
    ...
    for batch_index in range(config.bs):
        ...
        if is_train_trial:
            ...
            inputs[batch_index, config.nbstimbits + 2] = (cue_pair[1] - cue_pair[0])/(nbcues - 1) * (np.random.random() + 1)
        else:
