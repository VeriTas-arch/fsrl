"""Paired final-checkpoint training for three vector-modulation networks."""
...
def train_one(seed: int, source: dict) -> dict:
    ...
    with ProspectiveRun.start(
            ...
            for step in range(1500):
                ...
                if warm_seconds is None:
                    ...
                    if tensor_hashes(model) != before:
                        raise RuntimeError(
                            "warmup changed vector-modulation parameters"
                        )
                    del warm
                ...
                if not bool(torch.isfinite(result.loss)):
                    raise RuntimeError("nonfinite vector-modulation loss")
                fast = result.fast_weights.detach()
                head = model.h2modulation
                row = {
                    "step": step,
                    ...
                    "modulation_weight_frobenius": float(
                        torch.linalg.matrix_norm(head.weight.detach())
                    ),
                    "modulation_bias_frobenius": float(
                        torch.linalg.vector_norm(head.bias.detach())
                    ),
        ...
        metadata = {
            **identity,
            "runtime": runtime,
            "initial_vector_head": "zeros",
            "final_model": tensor_hashes(model),
            "parameters": sum(value.numel() for value in model.parameters()),
            "final_eta": float(model.etaet.detach()),
            "final_model_summary": {
                "modulation_weight_frobenius": float(
                    torch.linalg.matrix_norm(model.h2modulation.weight.detach())
                ),
                "modulation_bias_frobenius": float(
                    torch.linalg.vector_norm(model.h2modulation.bias.detach())
                ),
            },
            "stream_fingerprint": digest.hexdigest(),
            "optimizer_steps": {
