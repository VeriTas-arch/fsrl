"""Locked generic-first and clean/folded/noisy Liu evaluation."""
...
def load_model(seed: int, lock: dict):
    ...
    if tensor_hashes(model) != row["metadata"]["final_model"]:
        raise RuntimeError("loaded M2-alpha tensors differ")
...
def evaluate_generic_one(seed: int, panel: int, source: dict, lock: dict) -> dict:
    ...
    if tensor_hashes(model) != before:
        raise RuntimeError("generic evaluation changed M2-alpha")
    return result
...
def evaluate_liu_one(
    ...
    if tensor_hashes(adapter) != before:
        raise RuntimeError("Liu evaluation changed M2-alpha adapter")
    return result
