...
READINESS_PATH = resolve_record(
    "benchmarks/magnitude_placement_behavior_v1_1_collection_readiness.json"
)
RAW_SCHEMA_PATH = resolve_record(
    "benchmarks/magnitude_placement_behavior_v1_1_raw.schema.json"
)
REPAIR_PATH = resolve_record(
    "benchmarks/magnitude_placement_behavior_v1_1_collection_readiness_repair1.json"
)
MANIFEST_PATH = resolve_record(
    "benchmarks/magnitude_placement_behavior_v1_1_randomization.json.gz"
)
RESULT_PATH = resolve_record(
...
def validate_codebooks(protocol: dict) -> dict:
    ...
    gates = {
        ...
        "cross_hamming_4_to_12": int(np.min(cross)) == 4 and int(np.max(cross)) == 12,
...
def render_trial_svg(trial: dict, protocol: dict, readiness: dict) -> str:
    ...
    if (
        trial["code_left"] not in codebooks[codebook]
        or trial["code_right"] not in codebooks[codebook]
    ):
    ...
    if trial["phase"] == "support":
        ...
        parts.append(
            _bar_svg(int(layout["right_bar_x"]), trial["bar_right_level"], renderer)
        )
...
def _cell_schedule(readiness: dict) -> list[str]:
    ...
    for group in range(30):
        rng = np.random.default_rng(
            child_seed(readiness, f"group-{group + 1:02d}", "cell_order")
        )
        order = list(cells)
        rng.shuffle(order)
...
def _role_map(protocol: dict, readiness: dict, slot_id: str, codebook: str) -> dict:
    ...
    rng = np.random.default_rng(
        child_seed(readiness, slot_id, f"{codebook}_role_mapping")
    )
    order = rng.permutation(len(code_ids))
    return {
        role: code_ids[int(index)] for role, index in zip(roles, order, strict=True)
    }
...
def _support_trials(
    ...
    gaps = dict(zip(relations, inherited[f"assignment_{condition}_gaps"], strict=True))
...
def validate_manifest(manifest: dict, protocol: dict, readiness: dict) -> dict:
    ...
    for slot in slots:
        ...
        for session_index, session in enumerate(slot["sessions"], 1):
            ...
            session_maps_valid &= (
                session["condition"] == expected_order[session_index - 1]
            )
            session_maps_valid &= (
                session["codebook"] == expected_mapping[session["condition"]]
            )
            ...
            for block in range(1, 11):
                query_complete &= {
                    trial["pair"] for trial in query if trial["block_index"] == block
                } == pairs
            for trial in support:
                code_assignment_valid &= (
                    trial["code_left"] == role_map[trial["role_left"]]
                )
                code_assignment_valid &= (
                    trial["code_right"] == role_map[trial["role_right"]]
                )
...
def validate_session_bundle(
    ...
    for raw, scheduled in zip(bundle["trials"], trial_schedule, strict=True):
        ...
        schedule_exact &= (
            raw["trial_index_within_block"] == scheduled["trial_index_within_block"]
        )
        ...
        if raw["phase"] == "support":
            ...
            schedule_exact &= (
                raw["registered_gap_units"] == scheduled["registered_gap_units"]
            )
        else:
            bounds = (
...
def write_session_bundle_exclusive(path: Path, bundle: dict) -> dict:
    payload = (
        json.dumps(bundle, indent=2, sort_keys=True, allow_nan=False).encode("utf-8")
        + b"\n"
    )
    path.parent.mkdir(parents=True, exist_ok=True)
...
def run_readiness(
    ...
    with tempfile.TemporaryDirectory(
        prefix="magnitude-placement-readiness-"
    ) as directory:
        ...
        for slot in selected_slots:
            ...
            for session in slot["sessions"]:
                ...
                path = (
                    dry_run_root
                    / slot["slot_id"]
                    / f"session-{session['session_index']}.json"
                )
    ...
    source_files = (
        ROOT / "fsrl" / "experiments" / "human" / "magnitude_collection.py",
    ...
    gates = {
        ...
        "collection_remains_no_go": readiness["readiness_decision"]["collection_status"]
        == "NO_GO"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if arguments.command == "manifest":
        ...
        print(
            json.dumps(
                {"path": str(arguments.output), "sha256": file_sha256(arguments.output)}
            )
        )
        return 0
