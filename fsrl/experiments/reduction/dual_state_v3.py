...
def item_history_before(evidence: np.ndarray) -> np.ndarray:
    ...
    return np.concatenate(
        (np.zeros((1, evidence.shape[1])), np.cumsum(increments, axis=0)[:-1])
    )
...
def evaluate_fold(
    ...
    *,
    ...
    states, evidence, history, targets, episode_index = flatten_item_transitions(
        records
    )
    ...
    for record in records:
        ...
        terminal_history_norm.append(
            float(np.linalg.norm(np.sum(record.evidence**2, axis=0)))
        )
    ...
    remote = {
        ...
        "frozen_v2_remote_mse": float(
            v2_metrics["remote_reassembly"]["candidate_remote_mse"]
        ),
    }
    history_metrics = {
...
def bind_item_rollout(parameters: ItemHistoryParameters):
    ...
    v2.scalar_history_rollout_batch = lambda evidence, _parameters: (
        item_history_rollout_batch(evidence, parameters)
    )
    try:
        yield
    finally:
...
def build_result(
    ...
    result = {
        ...
        "final_parameters": {
            "A": final_parameters.A.tolist(),
            "B": final_parameters.B.tolist(),
        },
