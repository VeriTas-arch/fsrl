...
from fsrl.experiments.single_p_time_role_propagated.baseline import _decision
from fsrl.experiments.single_p_time_role_propagated.budgets import (
    cross_entropy_budget,
    margin_budget,
    probability_budget,
)
from fsrl.experiments.single_p_time_role_propagated.direct import (
    assert_bitwise_equal,
    clean_inputs,
)
from fsrl.experiments.single_p_time_role_propagated.locks import (
    reference,
    verify_reference,
)
from fsrl.experiments.single_p_time_role_propagated.protocol import PROTOCOL
from fsrl.experiments.single_p_time_role_propagated.storage import (
    deterministic_npz_bytes,
)
from fsrl.experiments.single_p_time_role_propagated.trajectory import regression_summary
...
def test_probability_and_ce_budgets_propagate_margin_tolerance():
    reference = np.asarray([-2.0, 0.0, 3.0])
    changed = reference + 0.9 * margin_budget(reference)
    for temperature in (1.0, 0.25):
        parent_probability = 1.0 / (1.0 + np.exp(-reference / temperature))
        direct_probability = 1.0 / (1.0 + np.exp(-changed / temperature))
        assert np.all(
            np.abs(direct_probability - parent_probability)
            <= probability_budget(reference, temperature)
        )
        parent_ce = np.logaddexp(0.0, -reference / temperature)
        direct_ce = np.logaddexp(0.0, -changed / temperature)
        assert np.all(
            np.abs(direct_ce - parent_ce)
            <= cross_entropy_budget(reference, temperature)
        )
