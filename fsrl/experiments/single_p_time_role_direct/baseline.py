...
from fsrl.experiments.single_p_time_role.analysis import validated_replay_error
...
def _strict_error(observed: np.ndarray, expected: np.ndarray, name: str) -> float:
    ...
    if left.shape != right.shape or not np.array_equal(
        ...
        raise RuntimeError(f"external {name} metadata differs")
    finite = np.isfinite(right)
    error = float(np.max(np.abs(left[finite] - right[finite])))
    if error > 1e-10:
        raise RuntimeError(f"external {name} error exceeds 1e-10: {error}")
    return error


def _external_errors(direct: dict, parent: dict) -> dict[str, float]:
    errors = {
        "generic_margin": validated_replay_error(
            direct["generic"]["margins"], parent["generic"]["margins"]
        ),
        "liu_margin": validated_replay_error(
            direct["liu"]["bundles__intact__logits"],
            parent["liu"]["bundles__intact__logits"],
        ),
        "generic_ce": _strict_error(
            direct["generic"]["ce"], parent["generic"]["ce"], "generic CE"
        ),
    }
    direct_probability = probabilities(direct)
    parent_probability = probabilities(parent)
    for name in direct_probability:
        errors[f"probability/{name}"] = _strict_error(
            direct_probability[name], parent_probability[name], name
        )
    return errors
...
def _pair(seed: int, panels: dict[str, dict], raws: dict[str, dict]) -> dict:
    ...
    means = {name: panel_mean(draws)[0] for name, draws in condition_draws.items()}
    ni = {cell: panel_mean([values[cell] for values in ni_panel])[0] for cell in CELLS}
    return {
        "panels": panels,
...
def _compare_parent(pairs: dict, parent: dict, outcome: str) -> dict:
    direct_points = _leaf_values(pairs, "point")
    parent_points = _leaf_values(parent["pairs"], "point")
    if direct_points.keys() != parent_points.keys():
        raise RuntimeError("external summary point inventory differs")
    point_error = max(
        abs(direct_points[name] - parent_points[name]) for name in direct_points
    )
    if point_error > 1e-8:
        raise RuntimeError(f"external summary point error exceeds 1e-8: {point_error}")
    ...
    return {
        "maximum_summary_point_absolute_error": point_error,
        "classification_leaves_compared": len(direct_flags),
        "summary_points_compared": len(direct_points),
                ...
                for panel in specification()["design"]["evaluation_panels"]:
                    ...
                    for condition in specification()["design"][
                        ...
                        for cell, settings in CELLS.items():
                            ...
                            errors = _external_errors(first_external, parent_raw)
                            ...
                            torch.cuda.empty_cache()
        pairs = {
            str(seed): _pair(seed, panels_by_seed[str(seed)], raws_by_seed[str(seed)])
            for seed in specification()["design"]["network_seeds"]
        }
        outcome = _aggregate_outcome(pairs)
        parent = load_json(verify_reference(lock["parents"]["parent_result"]))
        compatibility = _compare_parent(pairs, parent, outcome)
        manifest = {
            "schema_version": 1,
