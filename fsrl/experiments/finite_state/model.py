"""Actual finite-state support writes with explicit, independent rounding draws."""

import torch
from torch import nn

from fsrl.core.sequence import RecurrentSequence
from fsrl.experiments.memory_structure.model import Rollout, objective, read_queries
from fsrl.experiments.training_strategy.execution import PROFILE
from fsrl.infra.runtime import compile_module


def stochastic_store(proposal, uniforms, states):
    """Identity STE; exact grid-valued forward, no continuous residual state."""
    spacing = 100.0 / (states - 1)
    lower = torch.floor(proposal / spacing)
    residual = proposal - lower * spacing
    index = lower + (uniforms * spacing < residual).to(proposal.dtype)
    rounded = index * spacing
    return rounded.detach() + (proposal - proposal.detach())


class FiniteSequence(nn.Module):
    """Only the accepted support state is rounded; the cell ABI is unchanged."""

    def __init__(self, cell, states):
        super().__init__()
        if states is not None and (states < 3 or states % 2 != 1):
            raise ValueError("finite state count must be odd and include zero")
        self.cell = cell
        self.states = states

    def forward(self, inputs, hidden, eligibility, weights, uniforms):
        cost = weights.new_zeros(weights.shape[0])
        for index, current in enumerate(inputs.unbind(0)):
            _, _, _, hidden, eligibility, proposal = self.cell(
                current, hidden, eligibility, weights
            )
            updated = proposal
            if self.states is not None:
                updated = stochastic_store(proposal, uniforms[index], self.states)
            actual = ((updated - weights) * self.cell.alpha).abs().mean((1, 2))
            if self.states is None:
                increment = actual
            else:
                # Conditional E[abs(alpha * (Q(proposal)-P))] equals this value
                # when P is on the grid. Retain the realized forward cost while
                # differentiating this conditional expectation, not sign(0) STE.
                expected = ((proposal - weights) * self.cell.alpha).abs().mean((1, 2))
                increment = actual.detach() + (expected - expected.detach())
            cost = cost + increment
            weights = updated
        return weights, cost


def sequences(backbone, states, *, compiled=False):
    support, query = FiniteSequence(backbone, states), RecurrentSequence(backbone)
    if compiled:
        support = compile_module(support, PROFILE)
        query = compile_module(query, PROFILE)
    return support, query


def rollout(backbone, local, seqs, batch, states, rounding_seed):
    subjects = batch.support_inputs.shape[2]
    hidden = backbone.initial_hidden(subjects)
    eligibility = backbone.initial_eligibility(subjects)
    weights = backbone.initial_fast_weights(subjects)
    blank = batch.support_inputs.new_zeros(
        2, subjects, backbone.model_config.input_size
    )
    _, _, _, _, _, weights = seqs[1](blank, hidden, eligibility, weights, True)
    state = local.initial_state(subjects)
    rng = torch.Generator(device=weights.device).manual_seed(rounding_seed)
    writes = []
    first = weights
    for index, inputs in enumerate(batch.support_inputs.unbind(0)):
        shape = (inputs.shape[0], *weights.shape)
        if states is None:
            uniforms = weights.new_zeros(())
        else:
            uniforms = torch.rand(shape, generator=rng, device=weights.device)
        weights, amount = seqs[0](inputs, hidden, eligibility, weights, uniforms)
        writes.append(amount)
        state = local.write(
            state, inputs[0, :, : 2 * local.cue_size], batch.local_evidence[index]
        )
        if index == 0:
            first = weights
    logits, global_logits = read_queries(
        backbone, local, seqs[1], batch, weights, state
    )
    per_trial = torch.stack(writes)
    cost = per_trial.sum(0) / (
        batch.support_inputs.shape[0] * batch.support_inputs.shape[1]
    )
    return Rollout(logits, global_logits, weights, state, first), cost, per_trial


def update(backbone, local, seqs, batch, optimizer, spec, states, coefficient, draw):
    optimizer.zero_grad(set_to_none=True)
    result, cost, _ = rollout(backbone, local, seqs, batch, states, draw)
    base, ce = objective(result, batch, spec["optimization"]["fast_weight_penalty"])
    loss = base + coefficient * cost.mean()
    loss.backward()
    limit = spec["optimization"]["gradient_clip"]
    torch.nn.utils.clip_grad_norm_(
        backbone.parameters(), limit, error_if_nonfinite=True
    )
    torch.nn.utils.clip_grad_norm_([local.raw_gain], limit, error_if_nonfinite=True)
    optimizer.step()
    return {
        "loss": float(loss.detach()),
        "ce": float(ce.detach()),
        "cost": float(cost.detach().mean()),
    }
