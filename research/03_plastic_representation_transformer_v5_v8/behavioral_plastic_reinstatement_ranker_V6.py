"""
V6 behavioral plastic-reinstatement ranker.
...
class Config:
    ...
    pe: int = 50
    save_every: int = 1000
    output_dir: str = "outputs_behavioral_plastic_v5"
    device: str = "auto"
    amp: bool = False
    ...
    hidden_readout_gain: float = 0.15
    ...
    lambda_learning_aux: float = 0.30
    lambda_axis_margin: float = 0.05
    learning_margin: float = 0.15
...
def run_learning_phase(config: Config, net: PlasticReinstatementRanker, subject_z: torch.Tensor, true_scores: torch.Tensor,
    ...
    """Run the observed learning phase.

    V6 deliberately uses local supervision only for pairs that are actually shown
    during learning. The auxiliary and margin losses below never evaluate
    unobserved pairs, so they cannot leak the answers for the all-pair test set.
    Observed-pair replay also samples only from relations already presented in
    the current episode.
    """
    ...
    def add_observed_pair_losses(item_i: torch.Tensor, item_j: torch.Tensor, relation: torch.Tensor, scale: float) -> None:
        if scale <= 0:
            return
        if config.lambda_learning_aux <= 0 and config.lambda_axis_margin <= 0:
            return
        # The target is the sign of the observed learning relation. This is not a
        # hidden test label; it is the relation shown during the learning phase.
    ...
    for _block in range(config.n_learning_blocks):
        ...
        for k in range(n_edges):
            ...
            # Update plastic recurrent state from the actually observed relation.
            state, mod, eta_abs = net.observe_pair(codes, state, subject_z, item_i, item_j, rel, phase="learn", eta_scale=1.0)
            ...
            # Local scaffold: require the current plastic representation to judge
            # this observed relation. This uses no unobserved pair labels.
            add_observed_pair_losses(item_i, item_j, rel, scale=1.0)
            observed_buffer.append((item_i.detach(), item_j.detach(), rel.detach()))

            # Observed-pair replay/reinstatement. The buffer contains only shown
            # learning relations from this episode, not the hidden all-pair test set.
            if config.observed_replay_steps > 0 and observed_buffer:
                for _ in range(config.observed_replay_steps):
                    ...
                    state, rmod, reta_abs = net.observe_pair(
                        codes, state, subject_z, ri, rj, rr, phase="replay", eta_scale=config.observed_replay_eta_scale
                    )
                    mod_vals.append(rmod.mean())
                    eta_vals.append(reta_abs.mean())
                    plastic_vals.append(state.plastic.abs().mean())
                    trace_vals.append(state.trace.abs().mean())
                    add_observed_pair_losses(ri, rj, rr, scale=config.replay_aux_scale)

            # Optional free-form reinstatement from V5. Keep default at zero; use
            # observed-pair replay above as the main behaviorally grounded route.
...
def evaluate_paper_task(config: Config, net: PlasticReinstatementRanker) -> dict:
    ...
    summary = {
        ...
        "model_type": "v6_observed_replay_plastic_rank_axis_no_direct_item_writer",
...
def train(config: Config) -> PlasticReinstatementRanker:
    ...
    with open(out / "config_v6.json", "w") as f:
    ...
    log("[setup] V6: behavioral task + observed-pair replay scaffold + plastic representation transformer; no direct item-state writer")
    ...
    for ep in iterator:
        ...
        if config.save_every > 0 and ep > 0 and ep % config.save_every == 0:
            torch.save({"model_state": net.state_dict(), "config": asdict(config)}, out / "net_v6.pt")
            log(f"[save] checkpoint: {out / 'net_v6.pt'}")
    torch.save({"model_state": net.state_dict(), "config": asdict(config)}, out / "net_v6.pt")
    if last is not None:
...
def parse_args() -> Config:
    p = argparse.ArgumentParser(description="V6 behavioral plastic-reinstatement ranker: observed-pair replay scaffold, no direct item-state writer.")
    ...
    p.add_argument("--output-dir", type=str, default="outputs_behavioral_plastic_v6")
    ...
    p.add_argument("--hidden-readout-gain", type=float, default=0.15)
    ...
    p.add_argument("--replay-aux-scale", type=float, default=0.50)
    ...
    p.add_argument("--lambda-learning-aux", type=float, default=0.30)
    p.add_argument("--lambda-axis-margin", type=float, default=0.05)
    p.add_argument("--learning-margin", type=float, default=0.15)
