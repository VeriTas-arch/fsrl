"""
V5 behavioral plastic-reinstatement ranker.
...
6. Optional internal reinstatement steps replay/recode item representations
   through the same recurrent/plastic pathway after observed learning pairs.

Outer-loop training is still supervised by the true ranking after the full
learning phase. True labels are never used for no-feedback test updates.
"""
...
class Config:
    ...
    paper_graph_train_prob: float = 0.40
    ...
    reinstatement_steps: int = 2
    reinstatement_eta_scale: float = 0.30
    test_reinstatement_steps: int = 1
...
def run_training_episode(config: Config, net: PlasticReinstatementRanker) -> EpisodeStats:
    ...
    if not torch.isfinite(loss):
        raise FloatingPointError(
            f"non-finite loss: choice={float(test['choice_loss'].detach())} entropy={float(test['entropy'].detach())} "
    ...
    return EpisodeStats(
        ...
        final_score_std=float(final_scores.std(dim=1).mean().detach()),
    )
...
def evaluate_paper_task(config: Config, net: PlasticReinstatementRanker) -> dict:
    ...
    summary = {
        ...
        "model_type": "v5_plastic_reinstatement_no_direct_item_writer",
...
def train(config: Config) -> PlasticReinstatementRanker:
    ...
    with open(out / "config_v5.json", "w") as f:
    ...
    log("[setup] V5: behavioral task + plastic representation transformer + active reinstatement; no direct item-state writer")
    ...
    log(f"[setup] reinstatement_steps={config.reinstatement_steps}; hidden_readout_gain={config.hidden_readout_gain}")
    ...
    fields = [
        ...
        "mean_mod_test", "mean_eta_abs_test", "mean_abs_plastic_test", "final_score_std",
    ]
    ...
    for ep in iterator:
        ...
        with open(out / "train_log.csv", "a", newline="") as f:
            csv.writer(f).writerow([
                ...
                stats.mean_mod_test, stats.mean_eta_abs_test, stats.mean_abs_plastic_test, stats.final_score_std,
            ])
        if ep % config.pe == 0 or ep == config.nbiter - 1:
            ...
            log(
                ...
                f"score_std={stats.final_score_std:.3f}"
            )
            start = time.time()
        if config.save_every > 0 and ep > 0 and ep % config.save_every == 0:
            torch.save({"model_state": net.state_dict(), "config": asdict(config)}, out / "net_v5.pt")
            log(f"[save] checkpoint: {out / 'net_v5.pt'}")
    torch.save({"model_state": net.state_dict(), "config": asdict(config)}, out / "net_v5.pt")
    if last is not None:
...
def parse_args() -> Config:
    p = argparse.ArgumentParser(description="V5 behavioral plastic-reinstatement ranker: no direct item-state writer.")
    ...
    p.add_argument("--output-dir", type=str, default="outputs_behavioral_plastic_v5")
    ...
    p.add_argument("--paper-graph-train-prob", type=float, default=0.40)
    ...
    p.add_argument("--reinstatement-steps", type=int, default=2)
    p.add_argument("--reinstatement-eta-scale", type=float, default=0.30)
    p.add_argument("--test-reinstatement-steps", type=int, default=1)
