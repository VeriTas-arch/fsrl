        ...
        [
            {
                ...
                "passed": human["status"]
                == human_registration["required_status"],
...
def _validate_checkpoint(
    checkpoint: Path, specification: dict, seed: int
) -> dict:
...
def compare_behavior_to_human(
    ...
    return {
        "passed": all(
            check["inside_human_bootstrap_95_interval"] for check in checks
        ),
        "rule": "all registered scalar metrics inside human bootstrap 95% intervals",
        "checks": checks,
        "model_beta_pair_class_counts": summary[
...
def _control_summary(behavior: dict) -> dict:
    ...
    return {
        ...
        / eligible,
        "mean_inter_subject_kendall_tau": summary[
...
def parse_args(args=None):
    parser = argparse.ArgumentParser(description="Run the frozen confirmation workflow.")
    parser.add_argument(
