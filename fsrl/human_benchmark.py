...
def load_human_cohort(
    ...
    *,
    ...
    if observed_sha256 != expected_sha256:
        raise ValueError(
            f"{cohort} source SHA-256 mismatch: {observed_sha256}"
        )
    ...
    for source_id, rows in sorted(grouped.items()):
        ...
        for pair_index, pair in enumerate(pairs):
            ...
            preference_value = (
                2.0 * first_choices / total_counts[pair_index] - 1.0
            )
...
def fit_source_beta_distribution(values: np.ndarray, *, clip: float = 0.01) -> dict:
    ...
    alpha, beta, _location, _scale = stats.beta.fit(
        clipped, floc=0.0, fscale=1.0
    )
...
def summarize_human_subjects(
    protocol: RankingProtocol, subjects: list[dict]
) -> dict:
    pairs = tuple(combinations(range(protocol.n_items), 2))
    ...
    for pair_index, pair in enumerate(pairs):
        ...
        source_fit = fit_source_beta_distribution(
            pair_accuracy[analysis, pair_index]
        )
    ...
    return {
        ...
        "mean_self_consistency_coefficient": mean(
            "self_consistency_coefficient"
        ),
...
def bootstrap_human_summary(
    subjects: list[dict], *, seed: int, samples: int
) -> dict:
    if samples < 1:
    ...
    for _ in range(samples):
        ...
        for name in (
            ...
            metrics[name].append(float(np.mean([subject[name] for subject in eligible])))
...
def validate_paper_internal_checks(
    ...
    observed = {
        ...
        "stable_80_analysis_count": summary["stable_error_subject_prevalence"][
            "80"
        ]["analysis_count"],
        "stable_100_analysis_count": summary["stable_error_subject_prevalence"][
            "100"
        ]["analysis_count"],
    }
...
def build_human_benchmark(
    ...
    *,
    ...
    reproduction = validate_paper_internal_checks(
        combined_summary, combined, published
    )
...
def parse_args(args=None):
    ...
    parser.add_argument("--preregistered", type=Path, default=DEFAULT_PREREGISTERED_PATH)
